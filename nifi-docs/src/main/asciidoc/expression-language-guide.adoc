...
In this example, the value to be returned is the value of the "my attribute" attribute, if it exists. If that attribute
does not exist, the Expression Language will search through a hierarchy for a matching property. See <<expression-language-hierarchy>> 
for an explanation of the hierarchy.
...
NiFi will search for, and return, the first occurance of a matching property. If no matching property is found, `null` is returned.
