...
*Description*: [.description]#The `padLeft` function prepends the given padding character (or `'_'`, if nothing is provided) to the argument `String` until the passed desired length is reached.

It returns the argument as is if its length is already equal or higher than the desired length. It returns `null` if the
argument string is not a valid attribute or in case of `DesiredLength` overflow.
...
- [.argName]#_PaddingCharacter_# : [.argDesc]#The optional character to pad with. `'_'` will be used if a `PaddingCharacter` is not provided. If a `String` is passed here, it's first character will be used.#
...
| `${greetings:padLeft(10, 'abc')}` | `aaaaahello`
|=======================================================================================
...
*Description*: [.description]#The `padRight` function appends the given padding character (or `'_'`, if nothing is provided) to the argument `String` until the passed desired length is reached.
...
| `${greetings:padRight(10, 'abc')}` | `helloaaaaa`
|=======================================================================================
