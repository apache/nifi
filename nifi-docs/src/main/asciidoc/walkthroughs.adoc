...
+
NOTE: The `nifi.cluster.load.balance.address=` entry must be manually populated here because it was added after the TLS Toolkit was last updated. The toolkit correctly populates the hostname in all other necessary properties.
...
** `nifi.cluster.load.balance.address=` -> `nifi.cluster.load.balance.address=node1.nifi`
...
** `nifi.cluster.load.balance.address=` -> `nifi.cluster.load.balance.address=node2.nifi`
...
** `nifi.cluster.load.balance.address=` -> `nifi.cluster.load.balance.address=node3.nifi`
