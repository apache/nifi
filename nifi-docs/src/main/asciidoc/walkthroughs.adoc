...
== Purpose
The intent of this document is to provide a canonical source of prescriptive instruction sets for common administrator and user tasks using Apache NiFi. It is intended to complement the <<overview.adoc#apache-nifi-overview,NiFi Overview>>, <<administration-guide.adoc#nifi-system-administrators-guide,NiFi System Administrator's Guide>>, and <<user-guide.adoc#apache-nifi-user-guide,NiFi User's Guide>>. Those documents provide extensive reference information about various features and configuration values, but they do not provide information on _how_ to accomplish tasks.
