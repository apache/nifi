...
+
image:images/install-download-link.png["Download site"]
...
+
image:images/verify-release-gpg-and-checksums.png["Verifying the GPG signature and checksums]
...
+
image:images/nifi-home-dir-listing.png["NiFi Home directory contents"]
...
+
image:images/nifi-app-log-ui-available.png["NiFi application log listing available URLS"]
...
+
image:images/nifi-application-running-browser.png["NiFi application running in browser"]
...
image:images/nifi-tls-toolkit-standalone-cert-diagram.png["NiFi TLS Toolkit Standalone Certificate Diagram"]
...
* `ls -alGh` -- Lists the contents of the directory (optional)
+
image:images/nifi-toolkit-home-dir-listing.png["NiFi Toolkit Home directory contents"]
...
image:images/browser-warning-insecure-site.png["Browser warning on untrusted server TLS certificate"]

The browser prompting for a client certificate to present

image:images/browser-present-client-cert.png["Browser prompting for client certificate to present"]

The application running. Note the user's DN in the top-right corner

image:images/nifi-running-tls-client-certificate.png["NiFi running with user logged in by client certificate]
--
...
image:images/keychain-access-certificate-listing.png["Keychain Access listing certificates"]

* Expand the *Trust* section in the dialog
* Change *Secure Sockets Layer (SSL)* to *Always Trust*

image:images/keychain-access-trust-certificate.png["Trusting the CA certificate for TLS/SSL"]

* Close the dialog
* Restart the browser
* Navigate to link:https://localhost:9443/nifi[`https://localhost:9443/nifi`^]

image:images/nifi-trusted-server-certificate.png["Browser showing trusted NiFi server certificate"]
--
...
image:images/nifi-tls-standalone-external-certificate-diagram.png["NiFi TLS Standalone Provided Certificate Diagram"]
...
image:images/nifi-cluster-tls-toolkit-certificate-diagram.png["NiFi Cluster with TLS Toolkit Certificates Diagram"]
...
+
image:images/authorizers-xml-initial-user-identities.png["authorizers.xml with Initial User Identities"]
...
+
image:images/authorizers-xml-initial-node-identities.png["authorizers.xml with Initial Node Identities"]
...
image:images/nifi-secure-cluster-no-permissions.png["NiFi secure cluster running with no Initial Admin Identity permissions"]

_The cluster status from the global menu at the top right_

image:images/nifi-secure-cluster-status.png["NiFi secure cluster status pane"]
--
...
image:images/nifi-secure-cluster-permissions.png["NiFi secure cluster running with Initial Admin Identity permissions"]
--
