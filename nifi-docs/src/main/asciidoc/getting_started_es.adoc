...
Esta guía fue escrita para aquellos usuarios que nunca an usado, tienen poco uso o solo hicieron algunas cosas especificas en NIFI.
Esta Guía no es para solo usarla como un manual o para referencia. El link:user-guide.html[User Guide] da más información y fue creada
para dar más recursos. Esta guía solo da bastante información para poder entender como usar NIFI rapido, para poder crear flujo de datos poderosos y ágil.

Alguna de la información aplica solamente a los usuarios que estan usando NIFI por primera vez cuando otra información puede aplicar
para los que an usando NIFI anteriormente. Poreso esta guía tiene diferente secciónes que no puede ser importante para algunos. Puedes brincar
a cualquier sección que te aplica más.

Esta guía si anticipa que el usuario entienda que lo que NIFI es y lo que no es. Este tipo de información puede
ser encontrada en la documentación link:overview.html[Overview]


terminología usada en esta guía
------------------------------

Para poder hablar sobre NIFI, ahi algunos términos que el usuario deveria aprender. Vamos a explicar que esos términos más especificos a NIFI aqui.


*FlowFile*: Cada piesa de "datos de el usuario" (ejemplo: los datos que el usuario trae a NIFI para procesar y distribuir) es referido como un FlowFile.
Un FlowFile esta hecho de dos partes: Atributos y contenido. El contenido son los datos de el usuario mismo. Los atributos son valores importantes que son
asociados con los datos de el usuario.

*Processor*: El procesor es el componente de NIFI que es responsable por crear, mandar, recivir, transformar, asignación de ruta, dividir,
juntar y procesar FlowFiles. Es la parte más importante disponible a los usuarios de NIFI para construir sus flujo de datos.

Descargar y Instalar NIFI
-------------------------------

NIFI puede ser descargado de:http://nifi.apache.org/download.html[NiFi Downloads Page]. Ahi dos opciones con los empaque: el:
 "tarball" que es más para Linux y el zip es más para los usuarios de Windows. Usuarios de MAC OSX pueden usar el tarball o pueden
 instalar usando Homebrew.

Para instalar usando Homebrew, core este comando 'brew install nifi'.

Para usuarios que no estan usando OSX o no tienen Homebrew instalado, despues de descargar la version de NiFi que seas usar, puedes
extractar los archivos en el sitio donde quieres corer el programa.

Para más información como configurar NIFI (Por ejemplo, seguridad, donde vas a guardar los datos o el porte donde NIFI va a correr) ve a:administration-guide.html[Admin Guide].


Empesando NiFi
-------------

Despues que descargastes y instalastes NIFI como explicado anteriormente, puede ser empesado
usando el mechanismo más apropiado para tu sistema operativo.
...
=== Para los usuarios de Linux/Mac OSX
...
=== Sacar más  información de un Procesor
...
Que Procesores estan disponible
-----------------------------
