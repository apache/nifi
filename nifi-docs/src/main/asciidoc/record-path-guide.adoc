...
method of providing repeatable unique identifiers to record-oriented data. A common use case for UUID v5 is to assign a
UUID v4 to a particular data source or data set and then use that as the namespace for all of the records that originate
from that.
