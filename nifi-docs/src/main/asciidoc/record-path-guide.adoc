...
Injects the current date and time into the specified date record field. For example, given a schema such as:
...
`now(/date)` will inject a Java Date object into that field with the default GMT/UTC timezone.

Note: Java Date objects internally represent date and time as the time since the epoch with a UTC timezone. Java Date
objects do not have their own internal concept of timezones and work against that offset. The date format functionality
can convert this value into a String that is relative to a specified timezone.
