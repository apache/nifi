...
----------------------------------------
The goal of this paragraph is to provide a 20,000 foot view of NiFi's cornerstones, allowing readers to efficiently understand the big picture of Apache NiFi, and in the meantime, pointing out the most interesting focal points for readers to zoom in as needed. The key features are categorized as: flow management, ease of use, security, extensible architecture, and flexible scaling model.
...
Security::
    ...
    Multi-tenant Authorization;;
        The authority level of a given dataflow applies to each component, allowing the admin user to have fine grained level of access control. This in turn means each NiFi cluster is capable of handling the requirements of one or more organizations. Comparing to completely isolated topologies, multi-tenant authorization enables a self-service model for dataflow management, allowing each team/organization to manage their dataflows with a full awareness for the rest of the flow that they don't have access to. 
