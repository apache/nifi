...
image:images/restricted.png["Restricted"]
...
explicitly enable them for a subset of trusted users. Before a user is allowed to create and modify restricted components they must be granted access. Hovering over the image:images/restricted.png["Restricted"]
...
[[label]]
image:images/iconLabel.png["Label"]
...
image:images/add-processor-version-example.png["Add Processor Version Example"]

Each component displayed on the canvas also contains this information.

image:images/processor-version-information-example.png["Processor Version Information Example"]
...
image:images/add-processor-version-sort-filter.png["Add Processor Version Sort and Filter"]
...
+
image:images/processor-change-version.png["Processor Change Version"]
3. In the Component Version dialog, select the version you want to run from the Version drop-down menu.
+
image:images/component-version-dialog.png["Component Version"]
...
image:images/configure-processor-with-version-information.png["Configuration Version Requirements"]
...
image:images/processor-cs-version-match.png["Processor and Controller Service Version Match"]
...
image:images/processor-cs-version-mismatch-warnings.png["Processor and Controller Service Version Mismatch Warnings"]
...
image:images/processor-cs-version-mismatch-config.png["Processor and Controller Service Version Mismatch Property"]
...
image:images/primary-node-processor.png["Primary Node Processor"]

To quickly identify 'Primary Node' processors, the "P" icon is also shown in the Processors tab on the Summary page:

image:images/primary-node-processors-summary.png["Primary Node Processors in Summary Page"]
...
image:images/create-service-ssl-context.png["Create Service"]
...
image:images/el-param-support-help-text.png[Expression Language and Parameters Help Text]
...
image:images/parameter-contexts-selection.png["Global Menu - Parameter Contexts"]
...
image:images/parameter-contexts-settings.png["Parameter Contexts - Settings"]
...
image:images/add-parameter-during-parameter-context-creation.png[Add Parameter]

To add parameters to an existing Parameter Context, open the Parameter Context window and click the Edit button (image:images/iconEdit.png["Edit"]) in the row of the desired Parameter Context.

image:images/edit-parameter-context.png[Edit Parameter Context]
...
image:images/update-parameter-context.png[Update Parameter Context]
...
image:images/parameters-validate-affected-components.png[Validate Affected Components]
...
image:images/process-group-configuration-parameters.png[Configure Process Group Parameter Context]
...
image:images/process-group-parameter-context-menu.png[Process Group Parameter Context Menu]
...
image:images/context-menu-parameters-option.png[Context Menu Parameters Option]
...
image:images/existing-parameters-example.png[Existing Parameters]
...
image:images/autocomplete-parameter-example.png[Autocomplete Parameter Example]

Select `kafka.broker` and complete the entry with a closing curly brace `}`.

image:images/existing-parameter-selected.png[Existing Parameter Selected]
...
Parameters can also be created on the fly. For example, to create a parameter for the "Topic Name" property, select the "Convert to Parameter" icon (image:images/iconConvertToParameter.png["Convert to Parameter"]) in that property's row. This icon will only be available if the user has appropriate permissions to modify the Parameter Context (see <<accessing-parameters>> for more information).

image:images/convert-property-to-parameter.png[Convert Property to Parameter]

The Add Parameter dialog will open. Configure the new parameter as desired.

image:images/configure-parameter-on-the-fly.png[Configure Parameter On the Fly]
...
image:images/new-parameter-referenced.png[New Parameter Referenced]
...
image:images/reference-parameter-option.png[Reference Parameter Option]
...
image:images/reference-parameter-available-parameters.png[Reference Parameter Available Parameters]

Hovering over the question mark icon (image:images/iconInfo.png["Info"]) displays the parameter's description.
...
View and modify policies can also be set on individual parameter contexts to determine which users can view or add parameters to the context. Select "Parameter Contexts" from the Global Menu. Select the "Access Policies" button (image:images/iconAccessPolicies.png["Access Policies"]) in the row of the desired parameter context to manage these policies.

image:images/individual-parameter-context-polices.png[Individual Parameter Context Policies]
...
A user can only set the Parameter Context of a Process Group to one of the Parameter Contexts that the user has the view policy for. Additionally, in order to set the Parameter Context, the user must have the modify policy for the Process Group. The Process Group access policies can be managed by highlighting the Process Group and selecting the "Access Policies" button (image:images/iconAccessPolicies.png["Access Policies"]) from the Operate Palette.
...
image:images/el-support-help-text.png[Expression Language Help Text]
...
image:images/variables-context_menu-rpg.png["Variables in Context Menu for RPG"]

Select "Variables" from the Context Menu:

image:images/variables_window_empty.png["Empty Variables Window"]

"Variables" is also available in the right-click Context Menu when a process group is selected:

image:images/variables-context_menu-pg.png["Variables in Context Menu for PG"]

===== Creating a Variable

In the Variables window, click the `+` button to create a new variable.  Add a name:

image:images/variable-name.png["Variable Name Creation"]

and a value:

image:images/variable-value.png["Variable Value Creation"]

Select "Apply":

image:images/new_variable-apply.png["New Variable Applied"]
...
image:images/variable-putfile-property.png["Processor Property Using Variable"]
...
image:images/variable-overridden.png["Variable Overridden"]
...
image:images/variable_window-goto.png["Variable Go To"]

which will navigate to the Variables window for that process group:

image:images/variable_window-rpg.png["Variables Window for RPG"]
...
image:images/variable_insufficient-permissions.png["Insufficient Permissions to View Variables"]
...
image:images/variables-window_controller-services.png["Referencing Controller Services"]

Selecting the name of the controller service will navigate to that controller service in the Configuration window:

image:images/variable_nav-controller_services.png["Controller Service Using Variable"]
...
image:images/variables-window_unauthorized.png["Unauthorized Referencing Components"]

In the above example, the variable `property1` is referenced by a processor that "user1" is not able to view:

image:images/variable-unauthorized-ref-processor-canvas.png["Unauthorized Referencing Processor"]
...
image:images/controller-settings-selection.png["Global Menu - Controller Settings"]
...
image:images/controller-services-configure-buttons.png["Controller Services Buttons"]

You can obtain information about Controller Services by clicking the "Usage" and "Alerts" buttons in the left-hand column.

image:images/controller-services-info-buttons.png["Controller Services Information Buttons"]
...
image:images/configure-controller-service-settings.png["Configure Controller Service Settings"]
...
image:images/configure-controller-service-properties.png["Configure Controller Service Properties"]
...
image:images/process-group-configuration-options.png["Process Group Configuration Options"]
...
image:images/process-group-controller-services-scope.png["Process Group Controller Services Scope"]
...
+
image:images/process-group-configuration-window.png["Process Group Configuration Window"]
...
5. Perform any necessary Controller Service configuration tasks by clicking the Configure icon (image:images/iconConfigure.png["Configure"]) in the right-hand column.


[[Enabling_Disabling_Controller_Services]]
==== Enabling/Disabling Controller Services

After a Controller Service has been configured, it must be enabled in order to run. Do this using the "Enable" button (image:images/iconEnable.png["Enable Button"]) in the far-right column of the Controller Services tab. In order to modify an existing/running controller service, the DFM needs to stop/disable it (as well as all referencing reporting tasks and controller services). Do this using the "Disable" button (image:images/iconDisable.png["Disable Button"]). Rather than having to hunt down each component that is referenced by that controller service, the DFM has the ability to stop/disable them when disabling the controller service in question. When enabling a controller service, the DFM has the option to either start/enable the controller service and all referencing components or start/enable only the controller service itself.

image:images/enable-controller-service-scope.png["Enable Controller Service Scope"]
...
image:images/add-reporting-task-window.png["Add Reporting Task Window"]
...
image:images/reporting-tasks-edit-buttons.png["Reporting Tasks Edit Buttons"]
...
image:images/reporting-tasks-info-buttons.png["Reporting Tasks Information Buttons"]
...
image:images/configure-reporting-task-settings.png["Configure Reporting Task Settings"]
...
image:images/configure-reporting-task-properties.png["Configure Reporting Task Properties"]
...
When you want to run the Reporting Task, click the "Start" button (image:images/iconStart.png["Start Button"]).
...
image:images/processor-connection-bubble.png["Processor with Connection Bubble"]
...
image:images/file_expiration_clock.png["File Expiration Indicator"]
...
image:images/back_pressure_indicators.png["Back Pressure Indicator Bars"]

Hovering your mouse over a bar displays the exact percentage.

image:images/back_pressure_indicator_hover.png["Back Pressure Indicator Hover Text"]

When the queue is completely full, the Connection is highlighted in red.

image:images/back_pressure_full.png["Back Pressure Queue Full"]
...
image:images/load_balance_compression_options.png["Load Balance Compression Options"]
...
When a load balance strategy has been implemented for a connection, a load balance indicator (image:images/iconLoadBalance.png["Load Balance Icon"]) will appear on the connection:

image:images/load_balance_configured_connection.png["Connection Configured with Load Balance Strategy"]
...
image:images/load_balance_distributed_connection.png["Distributed Load Balance Connection"]
...
image:images/load_balance_active_connection.png["Active Load Balance Connection"]
...
image:images/summary_connections.png["NiFi Summary Connections"]

This will open the Cluster Connection Summary dialog, which shows the data on each node in the cluster:

image:images/cluster_connection_summary.png["Cluster Connection Summary Dialog"]
...
*Transport Protocol*: On a Remote Process Group creation or configuration dialog, you can choose Transport Protocol to use for Site-to-Site communication as shown in the following image:images/

image:images/configure-remote-process-group.png["Configure Remote Process Group"]
...
image:images/add-input-port-S2S.png["Add Input Port for Site-to-Site"]
...
image:images/add-output-port-S2S.png["Add Output Port for Site-to-Site"]
...
image:images/buttonStart.png["Start"]
) in the
...
image:images/buttonStop.png["Stop"]
...
clicking the "Enable" button (image:images/buttonEnable.png["Enable"]
...
image:images/buttonDisable.png["Disable"]
...
image:images/remote-group-ports-dialog.png["Remote Process Group Ports"]
...
image:images/component-linking-processor.png["Component Linking Processor Example"]
...
image:images/align-vertically-before.png["Align Vertically Example Before"]

and select "Align vertically" to achieve these results:

image:images/align-vertically-after.png["Align Vertically Example After"]
...
image:images/align-horizontally-before.png["Align Horizontally Example Before"]

and select "Align horizontally" to achieve these results:

image:images/align-horizontally-after.png["Align Horizontally Example Before"]
...
Additionally, details for a Flowfile in the listing can be viewed by clicking the "Details" button (image:images/iconDetails.png["Details"]) in the left most column. From here, the FlowFile details and attributes are available as well as buttons for
...
image:images/stats-history.png["Status History"]
...
image:images/registry-clients-tab.png["Registry Clients Tab"]

In the Add Registry Client window, provide a name and URL.

image:images/add-registry-client.png["Add Registry Client Dialog"]

Click "Add" to complete the registration.

image:images/registry-client-added.png["Registry Client Added"]
...
** image:images/iconUpToDate.png["Up to date"]
	*Up to date*: The flow version is the latest.

** image:images/iconLocallyModified.png["Locally Modified"]
	*Locally modified*: Local changes have been made.

** image:images/iconStale.png["Stale"]
	*Stale*: A newer version of the flow is available.

** image:images/iconLocallyModifiedStale.png["Locally Modified and Stale"]
	*Locally modified and stale*: Local changes have been made and a newer version of the flow is available.

** image:images/iconSyncFailure.png["Sync Failure"]
...
image:images/version-states-display.png["Version States Displayed"]

Version state information is also shown in the "Process Groups" tab of the Summary Page.

image:images/version-state-summary-page.png["Version State in Summary Page"]
...
image:images/add-process-group-import.png["Import Process Group"]

Selecting the link will open the Import Version dialog.

image:images/import-version-dialog.png["Import Version Dialog"]
...
image:images/versioned-flow-imported.png["Versioned Flow Imported"]

Since the version imported in this example is the latest version (MySQL CDC, Version 3), the state of the versioned process group is "Up to date" (image:images/iconUpToDate.png["Up To Date Icon"]).  If the version imported had been an older version, the state would be "Stale" (image:images/iconStale.png["Stale Icon"]).
...
image:images/start-version-control.png["Start Version Control"]
...
image:images/save-flow-version-dialog.png["Save Flow Version Dialog"]

Select Save and Version 1 of the flow is saved.

image:images/versioned-process-group.png["Versioned Process Group"]

As the first and latest version of the flow, the state of the versioned process group is "Up to date" (image:images/iconUpToDate.png["Up To Date Icon"]).

NOTE: The root process group can not be placed under version control.

[[managing_local_changes]]
=== Managing Local Changes
When changes are made to a versioned process group, the state of the component updates to "Locally modified" (image:images/iconLocallyModified.png["Locally Modified Icon"]).  The DFM can show, revert or commit the local changes. These options are available for selection in the context menu when right-clicking on the process group:

image:images/local-changes-pg-selected.png["Local Changes PG Selected"]

or when right-clicking on the canvas inside the process group:

image:images/local-changes-pg-inside.png["Local Changes Inside PG"]
...
image:images/show-local-changes-dialog.png["Show Local Changes Dialog"]

You can navigate to a component by selecting the "Go To" icon (image:images/iconGoTo.png["Go To"]) in its row.
...
image:images/revert-local-changes-dialog.png["Revert Local Changes Dialog"]
...
image:images/save-flow-version-commit.png["Save Flow Version Commit"]

Local changes can not be committed if the version that has been modified is not the latest version.  In this scenario, the version state is "Locally modified and stale" (image:images/iconLocallyModifiedStale.png["Locally Modified and Stale"]).

=== Change Version
To change the version of a flow, right-click on the versioned process group and select "Version->Change version".

image:images/change-version.png["Change Version"]

In the Change Version dialog, select the desired version and select "Change":

image:images/change-version-dialog.png["Change Version Dialog"]

The version of the flow is changed:

image:images/flow-version-changed.png["Flow Version Changed"]
...
image:images/stop-version-control.png["Stop Version Control"]

In the Stop Version Control dialog, select "Disconnect".

image:images/stop-version-control-dialog.png["Stop Version Control Dialog"]

The removal of the process group from version control is confirmed.

image:images/disconnect-dialog.png["Disconnect Confirmation Dialog"]

image:images/process-group-version-control-stopped.png["Version Control Stopped on Process Group"]
...
image:images/pc1_parameters.png[PC1 Parameters]
...
image:images/nifi_pc1_parameters.png[NiFi PC1 Parameters]

The versioned flow is imported into the NiFi instance. The Parameter Context "PC1" now has the following parameters:

image:images/nifi_pc1_parameters_new.png[New NiFi PC1 Parameters]
...
image:images/rpg-variable.png["Root Process Group Defined Variable"]

A process group PG1 is created:

image:images/PG1_process_group.png["PG1 Process Group"]

The GetFile processor in PG1 references the variable "RPG_Var":

image:images/PG1_variable_ref_1.png["PG1 References RPG Variable"]

PG1 is saved as a versioned flow:

image:images/PG1_versioned_flow.png["PG1 Versioned Flow"]

If PG1 versioned flow is imported into this same NiFi instance:

image:images/PG1_imported_same.png["PG1 Imported to Same NiFi"]

the added GetFile processor will also reference the "RPG_Var" variable that exists in the root process group:

image:images/PG1_variable_ref_2.png["Both PG1 Reference RPG Variable"]

If PG1 versioned flow is imported into a different NiFi instance where "RPG_Var" does not exist:

image:images/PG1_imported_diff.png["PG1 Imported to Different NiFi"]

A "RPG_Var" variable is created in the PG1 process group:

image:images/PG1_variable_ref_PG.png["PG1 References PG Variable Copy"]
...
+
image:images/sys_admin-restricted-component-access-policy.png["Sys_admin Restricted Component Access Policy"]

* "test_user" has access to restricted components requiring 'read filesystem' and 'write filesystem'.
+
image:images/test_user-restricted-component-read-filesystem.png["Test_user Restricted Component Read Filesystem"]
+
image:images/test_user-restricted-component-write-filesystem.png["Test_user Restricted Component Write Filesystem"]
...
image:images/keytabCredentialsService-rpg.png["KeytabCredentialsService Controller Service RPG Level"]

KeytabCredentialService controller service is a restricted component that requires 'access keytab' permissions:

image:images/keytabcredentialsservice-permissions.png["KeytabCredentialService Required Permissions"]

Sys_admin creates a process group ABC containing a flow with GetFile and PutHDFS processors:

image:images/abc-restricted-component-flow.png["Restricted Component Flow"]

GetFile processor is a restricted component that requires 'write filesystem' and 'read filesystem' permissions:

image:images/getfile-permissions.png["GetFile Required Permissions"]

PutHDFS is a restricted component that requires 'write filesystem' permissions:

image:images/puthdfs-permissions.png["PutHDFS Required Permissions"]

The PutHDFS processor is configured to use the root process group level KeytabCredentialsService controller service:

image:images/puthdfs-properties.png["PutHDFS Properties"]

Sys_admin saves the process group as a versioned flow:

image:images/abc-versioned-flow.png["ABC Versioned Flow"]

Test_user changes the flow by removing the KeytabCredentialsService controller service:

image:images/puthdfs-no-kerberosCS.png["PutHDFS No Kerberos CS"]

If test_user chooses to revert this change:

image:images/test_user-revert-local-changes.png["Test_user Revert Local Changes]

the revert is successful:

image:images/revert-success.png["Revert Local Changes Successful"]

Additionally, if test_user chooses to import the ABC versioned flow:

image:images/test_user-import-abc-flow.png["Test_user Import Flow"]

The import is successful:

image:images/test_user-import-success.png["Test_user Import Successful"]
...
image:images/xyz-process-group.png["XYZ Process Group"]

Sys_admin creates a KeytabCredentialsService controller service at the process group level:

image:images/keytabCredentialsService-pg.png["KeytabCredentialsService Controller Service PG Level"]

The same GetFile and PutHDFS flow is created in the process group:

image:images/xyz-flow.png["XYZ Versioned Flow"]

However, PutHDFS now references the process group level controller service:

image:images/puthdfs-properties_2.png["PutHDFS Properties"]
...
image:images/test_user-revert-local-changes-2.png["Test_user Revert Local Changes"]
...
image:images/revert-failure.png["Revert Local Changes Fails"]

Similarly, if test_user tries to import the XYZ versioned flow:

image:images/test_user-import-xyz-flow.png["Test_user Import Flow"]

The import fails:

image:images/import-xyz-flow-fails.png["XYZ Import Fails"]
...
"Create Template" (image:images/iconNewTemplate.png["Create Template"]) button in the Operate Palette (See <<User_Interface>> for more information on the Operate Palette).
...
From the Operate Palette, click the "Upload Template" (image:images/iconUploadTemplate.png["Upload Template"]) button (see <<User_Interface>> for more information on the Operate Palette).  This will display the Upload Template
...
can be used to help find the appropriate Template if several are available. Then click the "Download" button (image:images/iconDownloadTemplate.png["Export"]). This will download the template as an XML file to your computer. This XML file can then be sent to others and imported into other instances of NiFi (see <<Import_Template>>).
...
[[event_details]]
=== Details of an Event
In the far-left column of the Data Provenance page, there is a "View Details" icon for each event (image:images/iconDetails.png["Details"]).
...
click on the "Show Lineage" icon ( image:images/iconLineage.png["Show Lineage", width=28] ) in the far-right column
of the Data Provenance table. This opens a graph displaying the FlowFile ( image:images/lineage-flowfile.png["FlowFile", width=32] ) and the
...
image:images/encrypted-wapr-hex.png["Encrypted provenance repository file on disk"]
...
image:images/encrypted-content-hex.png["Encrypted content repository file on disk"]
...
image:images/encrypted-flowfile-hex.png["Encrypted flowfile repository journal file on disk"]
