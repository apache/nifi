...
In a manner similar to the other encrypted repository implementations, the FlowFile repository can be encrypted at rest.

[WARNING]
.Performance
============
The performance guidance for Encrypted FlowFiles Repositories is the same as for the Encrypted Provenance Repository.  Notably, switching back and forth between implementations is not recommended.
============

==== What is it?

Each of the classes that interacts with disk streams for the FlowFile repository have been updated to include support for transparent encryption/decryption via cipher streams.

==== How does it work?

When activated, these streams use an AES cipher in AEAD mode to provide authenticated encryption.  Authenticated encryption ensures that encrypted data has not been altered, which
means that FlowFile data that has been tampered with is not loaded or used.

===== Keys and Key Rotation
The `StaticKeyProvider` implementation (as described in the Encrypted Provenance Repository section) is used for Encrypted FlowFile keys.

Keys can be rotated in the same manner as described in the Encrypted Provenance Repository section.

==== Potential Issues

In general, the guidance from other encrypted repository implementations applies to Encrypted FlowFile Repositories.  Specifically:

* Switching between unencrypted and encrypted repositories is not supported.
...
* Corruption -- when an encrypted FlowFile is closed cleanly or corrupted, an error is logged and the FlowFile is not loaded.
