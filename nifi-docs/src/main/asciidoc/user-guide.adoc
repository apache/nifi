...
When a component is stopped, it does not interupt the currently running tasks. This allows for the current execution to complete while no new
...
The number of tasks that are actively being terminated will be displayed in parenthesis next to the number of active tasks e.g. image:terminated-thread.png["Terminated-Threads"]. For example, if there is one active task at the time that Terminate is selected, this will diplay "0 (1)" - meaning
