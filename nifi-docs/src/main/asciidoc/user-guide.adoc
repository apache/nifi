...
NiFi also supports a `NoOpProvenanceRepository` implementation that does not store any provenance events.
...
The `NoOpProvenanceRepository` implementation:
...
To configure NiFi to use the `NoOpProvenanceRepository`, set in `nifi.properties`:
...
Note that disabling provenance storage will disable features relying on provenance data, such as lineage queries and event searches.
