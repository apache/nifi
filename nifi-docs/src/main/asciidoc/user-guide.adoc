...
`https://localhost:9443/nifi` in a web browser. The default configuration generates a username and password
...
The default URL for a NiFI instance is `https://localhost:9443/nifi`, which points to the root process group.  When a component is selected on the canvas, the URL is updated with the component's process group id and component id in the form `https://localhost:9443/nifi/?processGroupId=<UUID>&componentIds=<UUIDs>`.  In the following screenshot, the GenerateFlowFile processor in the process group PG1 is the selected component:
