...
|access the controller  |Allows users to view and modify the controller including Reporting Tasks, Flow Analysis Rules, Controller Services, and nodes in the cluster
...
The values of properties in the flow, including sensitive properties, can be parameterized using Parameters. Parameters are created and configured within the NiFi UI. Any property can be configured to reference a Parameter with the following conditions:
 ...
 - Parameters cannot be referenced in Reporting Tasks, Flow Analysis Rules or in controller-level Controller Services
...
[[Controller_Services_for_Reporting_Tasks_and_Flow_Analysis_Rules]]
==== Adding Controller Services for Reporting Tasks/Flow Analysis Rules

To add a Controller Service for a reporting task or a flow analysis rule, select Controller Settings from the Global Menu.

image:controller-settings-selection.png["Global Menu - Controller Settings"]

This displays the NiFi Settings window. The window has five tabs: General, Reporting Task/flow Analysis Rule Controller Services, Reporting Tasks, Flow Analysis Rules and Registry Clients. The General tab provides settings for the overall maximum thread counts of the instance.

image:settings-general-tab.png["Controller Settings General Tab"]

To the right of the General tab is the Reporting Task/Flow Analysis Rule Controller Services tab. From this tab, the DFM may click the `+` button in the upper-right corner to create a new Controller Service.
