...
|`vault.transit.path`|The HashiCorp Vault `path` specifying the Transit Secrets Engine (e.g., `nifi-transit`).  Valid characters include alphanumeric, dash, and underscore.|_none_
|===
...
|`vault.ssl.key-store-type`|Keystore type (JKS or PKCS12).  Required if the Vault server is TLS-enabled|_none_
...
|`vault.ssl.trust-store-type`|Truststore type (JKS or PKCS12).  Required if the Vault server is TLS-enabled|_none_
