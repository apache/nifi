...
|`vault.auth.props.file`|Filename of a properties file containing Vault authentication properties.  See the `Authentication-specific property keys` section of https://docs.spring.io/spring-vault/docs/2.3.x/reference/html/#vault.core.environment-vault-configuration" for all authentication property keys. If not set, this provider will be disabled.|_none_
|`vault.transit.path`|The HashiCorp Vault `path` specifying the Transit Secrets Engine (e.g., `nifi-transit`).|_none_
|===
...
|`vault.enabled.tls.cipher.suites`|A comma-separated list of the enabled TLS cipher suites|_none_
|`vault.enabled.tls.protocols`|A comma-separated list of the enabled TLS protocols|_none_
|`vault.keystore`|Path to a keystore.  Required if the Vault server is TLS-enabled|_none_
|`vault.keystoreType`|Keystore type (JKS or PKCS12).  Required if the Vault server is TLS-enabled|_none_
|`vault.keystorePasswd`|Keystore password.  Required if the Vault server is TLS-enabled|_none_
|`vault.truststore`|Path to a truststore.  Required if the Vault server is TLS-enabled|_none_
|`vault.truststoreType`|Truststore type (JKS or PKCS12).  Required if the Vault server is TLS-enabled|_none_
|`vault.truststorePasswd`|Truststore password.  Required if the Vault server is TLS-enabled|_none_
|===
