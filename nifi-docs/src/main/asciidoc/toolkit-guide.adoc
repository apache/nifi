...
==== AWS_KMS
This protection scheme uses AWS Key Management Service, or AWS KMS for short (https://aws.amazon.com/kms/) for encryption/decryption . All AWS KMS configuration/credentials is stored in the `bootstrap-aws.conf` file, as referenced in the `bootstrap.conf` of a NiFi or NiFi Registry instance.  Therefore, when using the AWS_KMS protection scheme, the `nifi(.registry)?.bootstrap.protection.aws.kms.conf` property in the `bootstrap.conf` specified using the `-b` flag must be available to the Encrypt Configuration Tool and must be configured as follows:
...
./../nifi-toolkit-1.14.0-SNAPSHOT/bin/encrypt-config.sh \
...
./../nifi-toolkit-1.14.0-SNAPSHOT/bin/encrypt-config.sh \
-b /Users/emilio.setiadarma/test-nifi-deploy/nifi-1.14.0-SNAPSHOT/conf/bootstrap.conf \
-n /Users/emilio.setiadarma/test-nifi-deploy/nifi-1.14.0-SNAPSHOT/conf/nifi.properties \
-S AWS_KMS \
-H AES_GCM \
