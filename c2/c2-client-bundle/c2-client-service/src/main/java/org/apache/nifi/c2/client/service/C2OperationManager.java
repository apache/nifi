...
public class C2OperationManager implements Runnable {
    ...
    public C2OperationManager(final C2Client client, final C2OperationHandlerProvider c2OperationHandlerProvider, final ReentrantLock heartbeatLock,
                              final OperationQueueDAO operationQueueDAO, final C2OperationRestartHandler c2OperationRestartHandler) {
        this.client = client;
    ...
    public void add(final C2Operation c2Operation) {
        try {
    ...
    private void processOperationsInLoop() {
        while (true) {
            ...
            final C2OperationHandler operationHandler = c2OperationHandlerProvider.getHandlerForOperation(operation).orElse(null);
            ...
            final C2OperationAck operationAck;
            try {
            ...
            final Optional<C2OperationState> restartState = initRestart(operation);
            ...
            try {
                final C2OperationState failedState = restartState.get();
    ...
    private void processRestartState() {
        final Optional<OperationQueue> operationQueue = operationQueueDAO.load();
    ...
    private void processRemainingOperations(final List<C2Operation> remainingOperations) {
        LOGGER.debug("Found remaining operations operations after restart. Heartbeats are stopped until processing is completed");
        heartbeatLock.lock();
        try {
            final List<C2Operation> mergedOperations = new LinkedList<>();
    ...
    private void processCurrentOperation(final C2Operation operation) {
        LOGGER.debug("Found operation {} to acknowledge to C2 server", operation);

        final C2OperationState c2OperationState = c2OperationRestartHandler.waitForResponse()
    ...
    private Optional<C2OperationState> initRestart(final C2Operation operation) {
        try {
            LOGGER.debug("Restart initiated");
            final OperationQueue operationQueue = OperationQueue.create(operation, c2Operations);
    ...
    private C2OperationState c2OperationState(final OperationState operationState) {
    ...
    private void sendAcknowledge(final C2OperationAck operationAck) {
        try {
    ...
    private boolean requiresRestart(final C2OperationHandler c2OperationHandler, final C2OperationAck c2OperationAck) {
        return c2OperationHandler.requiresRestart() && isOperationFullyApplied(c2OperationAck);
    }

    private boolean isOperationFullyApplied(final C2OperationAck c2OperationAck) {
