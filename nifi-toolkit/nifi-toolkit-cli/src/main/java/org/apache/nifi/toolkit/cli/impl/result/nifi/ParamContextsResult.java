...
public class ParamContextsResult extends AbstractWritableResult<ParameterContextsEntity> implements Referenceable {
    ...
    protected void writeSimpleResult(final PrintStream output) throws IOException {
        final Table table = new Table.Builder()
                ...
                .column("Sensitive Param Provider", 20, 60, false)
                .column("Non-sensitive Param Provider", 20, 60, false)
                .column("Description", 40, 60, true)
                .build();

        for (int i = 0; i < results.size(); i++) {
            final ParameterContextDTO r = results.get(i);
            final String sensitiveParameterProviderId = r.getSensitiveParameterProviderRef() == null ? "" : r.getSensitiveParameterProviderRef().getId();
            final String nonSensitiveParameterProviderId = r.getNonSensitiveParameterProviderRef() == null ? "" : r.getNonSensitiveParameterProviderRef().getId();
            ...
            table.addRow("" + (i+1), r.getId(), r.getName(), inheritedParamContexts, sensitiveParameterProviderId, nonSensitiveParameterProviderId, r.getDescription());
        }
