...
public class UploadNarBundles extends AbstractNiFiRegistryCommand<StringResult> {

    public UploadNarBundles() {
        super("upload-nars", StringResult.class);
    }

    @Override
    public String getDescription() {
        return "Performs a bulk upload of multiple NARs to the specified bucket in the registry. This command will look for " +
                "files with an extension of .nar in the specified directory, and if recurse (-r) is specified then it " +
                "will search child directories recursively.";
    }
    ...
    public StringResult doExecute(final NiFiRegistryClient client, final Properties properties)
        ...
        uploadNars(bundleVersionClient, bucketId, startDir, recursive, skipSha256, verbose, counter);

        return new StringResult("Uploaded " + counter.get() + " NARs successfully", getContext().isInteractive());
    }

    private void uploadNars(final BundleVersionClient bundleVersionClient, final String bucketId, final File directory,
                            final boolean recurse, final boolean skipSha256, final boolean verbose, final AtomicInteger counter) {

        for (final File file : directory.listFiles()) {
            if (file.isDirectory() && recurse) {
                uploadNars(bundleVersionClient, bucketId, file, recurse, skipSha256, verbose, counter);
            } else if (file.getName().endsWith(".nar")) {
                ...
                try (final InputStream bundleInputStream = new FileInputStream(file)) {
                    final BundleVersion createdVersion = bundleVersionClient.create(
                            bucketId, BundleType.NIFI_NAR, bundleInputStream, sha256);
                    ...
                    println("Error uploading NAR from " + file.getAbsolutePath());
                    if (verbose) {
