...
public class FileManagerTool extends AbstractAdminTool{
    ...
    void restore(String backupNiFiDirName, String rollbackNiFiDirName, String currentNiFiDirName, String bootstrapConfFileName){
        ...
        if(!StringUtils.isEmpty(currentNiFiDirName) && moveRepositories) {
    ...
    void install(final String installFileName, final String installDirName, final String currentNiFiDirName, final String bootstrapConfFileName, final Boolean overwriteConfigs){
        ...
        if(installFile.exists()){
            ...
            if(valid(installRootDir)){
                ...
                if(!StringUtils.isEmpty(currentNiFiDirName) && !StringUtils.isEmpty(bootstrapConfFileName)){
    ...
    void parseInstall(final CommandLine commandLine){
        ...
        final String bootstrapConfFileName = commandLine.hasOption(BOOTSTRAP_CONF) ? commandLine.getOptionValue(BOOTSTRAP_CONF) : !StringUtils.isEmpty(nifiCurrentDirName) ?
        ...
        if (!StringUtils.isEmpty(nifiCurrentDirName) && Files.notExists(Paths.get(nifiCurrentDirName))) {
            throw new ParseException("Current NiFi installation path does not exist: " + nifiCurrentDirName)
        }

        if(!StringUtils.isEmpty(nifiCurrentDirName) && !StringUtils.isEmpty(bootstrapConfFileName) && !supportedNiFiMinimumVersion(nifiCurrentDirName, bootstrapConfFileName, SUPPORTED_MINIMUM_VERSION)) {
            throw new UnsupportedOperationException("File Manager Tool only supports NiFi versions 1.0.0 or higher.")
        }

        install(installFileName, nifiInstallDirName, StringUtils.isEmpty(nifiCurrentDirName)? null : Paths.get(nifiCurrentDirName).toFile().getCanonicalPath(), bootstrapConfFileName, overwriteConfigs)
    ...
    void parseRestore(final CommandLine commandLine){
        ...
        restore(backupDirName, nifiRollbackDirName, StringUtils.isEmpty(nifiCurrentDirName)? null : Paths.get(nifiCurrentDirName).toFile().getCanonicalPath(), bootstrapConfFileName)
