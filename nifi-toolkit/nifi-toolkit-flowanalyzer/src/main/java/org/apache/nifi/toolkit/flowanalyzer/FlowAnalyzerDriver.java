...
public class FlowAnalyzerDriver {
    ...
    final static int CONST_ARGUMENT_LEN = 1;

    private static void printUsage() {
        System.out.println("This application seeks to produce a report to analyze the FlowFile.xml.gz");
        System.out.println(
                "Currently the reports supported by this application are Total Storage for all queues " +
                "backpressure, average storage of all queues back pressure, and min and max of all queues" +
                 "back pressure over the entire flow.");
        System.out.println("\n\n\n");
        System.out.println("Usage: flow-analyzer.sh <path to flowfile.xml.gz>");
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length != CONST_ARGUMENT_LEN) {
            printUsage();
            return;
        }
        ...
        System.out.println("Using flowfile=" + input);
        ...
        try {
            ...
            e.printStackTrace();
            System.out.println("Exception Occured while attempting to parse Flowfile.xml.  Cause: " + e.getCause());
            return;
        }
