...
public abstract class NiFiSystemIT implements NiFiInstanceProvider {
    ...
    // Cluster role value as reported by the REST API's NodeDTO. Defined here as a plain String, matching the wire
    // format - nifi-api has no equivalent type for cluster roles (unlike NodeConnectionState, which it does provide).
    ...
    protected boolean isCoordinatorElected() throws NiFiClientException, IOException {
        ...
        for (final NodeDTO nodeDto : clusterEntity.getCluster().getNodes()) {
            if (nodeDto.getRoles().contains(CLUSTER_ROLE_COORDINATOR) && nodeDto.getStatus().equals(NodeConnectionState.CONNECTED.name())) {
                return true;
            }
        }
