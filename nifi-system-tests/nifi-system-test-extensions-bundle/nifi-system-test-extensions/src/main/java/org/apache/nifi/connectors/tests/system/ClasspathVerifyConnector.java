 ...
 * Supports processor verification with property overrides, class loading through a Connector Method,
 * and consecutive Connector Method invocations that verify component instance state is preserved.
 */
public class ClasspathVerifyConnector extends AbstractConnector {
    ...
    public static final String STRATEGY_CONNECTOR_METHOD_STATE = "Connector Method State";
    public static final String CONNECTOR_METHOD_STEP = "Invoke loadClass Connector Method";
    public static final String CONNECTOR_METHOD_STATE_STEP = "Preserve Connector Method Component State";
    ...
    public static final ConnectorPropertyDescriptor VERIFICATION_STRATEGY = new ConnectorPropertyDescriptor.Builder()
            ...
            .description("Whether verification should call ProcessorFacade.verify, invoke the loadClass Connector Method, or verify Connector Method component state")
            .required(true)
            .type(PropertyType.STRING)
            .defaultValue(STRATEGY_PROCESSOR_VERIFY)
            .allowableValues(STRATEGY_PROCESSOR_VERIFY, STRATEGY_CONNECTOR_METHOD, STRATEGY_CONNECTOR_METHOD_STATE)
            .build();
    ...
    private List<ConfigVerificationResult> verifyConnectorMethodState(final ProcessorFacade processorFacade) {
        try {
            final Integer firstInvocationCount = processorFacade.invokeConnectorMethod("incrementInvocationCount", Map.of(), Integer.class);
            final Integer secondInvocationCount = processorFacade.invokeConnectorMethod("incrementInvocationCount", Map.of(), Integer.class);
            final boolean statePreserved = firstInvocationCount == 1 && secondInvocationCount == 2;
            return List.of(new ConfigVerificationResult.Builder()
                    .verificationStepName(CONNECTOR_METHOD_STATE_STEP)
                    .outcome(statePreserved ? Outcome.SUCCESSFUL : Outcome.FAILED)
                    .explanation("Connector Method invocation counts were " + firstInvocationCount + " and " + secondInvocationCount)
                    .build());
        } catch (final InvocationFailedException e) {
            return List.of(new ConfigVerificationResult.Builder()
                    .verificationStepName(CONNECTOR_METHOD_STATE_STEP)
                    .outcome(Outcome.FAILED)
                    .explanation("Failed to invoke stateful Connector Method: " + e.getMessage())
                    .build());
        }
    }
