...
class CollectPropertyDescriptorVisitors(ast.NodeVisitor):
    ...
    def __init__(self, module_string_constants):
    ...
    def resolve_dependencies(self, node: ast.AST):
        ...
        for dependency in node.elts:
            ...
            if not self.discovered_property_descriptors[variable_name]:
                self.logger.error(f"Not able to find actual property descriptor for {variable_name}, so not able to resolve property dependencies.")
            else:
    ...
    def visit_Assign(self, node: ast.AST):
        if self.assignment_is_property_descriton(node):
            self.logger.debug(f"Found PropertyDescriptor in the following assignment {node.targets[0].id}")
            if not node.value.keywords:
                self.logger.error(f"Not able to parse {node.targets[0].id} PropertyDescriptor as no keywords assignments used.")
            else:
                descriptor_info = {}
                for keyword in node.value.keywords:
                    key = keyword.arg
                    if key == 'dependencies':
                        self.logger.info(f"Resolving dependencies for {node.targets[0].id}.")
                ...
                self.discovered_property_descriptors[node.targets[0].id] = PropertyDescription(name=descriptor_info.get('name'),
