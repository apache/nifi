    ...
    public List<ConfigVerificationResult> verifyConfigurationStep(final String stepName, final List<PropertyGroupConfiguration> overrides, final FlowContext flowContext) {
        ...
        if ("Source".equals(stepName)) {
            ...
            results.add(new ConfigVerificationResult.Builder()
                .verificationStepName("Source Text Value")
                .outcome(ConfigVerificationResult.Outcome.SUCCESSFUL)
                .explanation("Source Text value: " + sourceText)
                .build());

            final boolean countFlowFiles = configContext.getProperty(SOURCE_STEP, SOURCE_GROUP, COUNT_FLOWFILES).asBoolean();
            results.add(new ConfigVerificationResult.Builder()
                .verificationStepName("Count FlowFiles Value")
                .outcome(ConfigVerificationResult.Outcome.SUCCESSFUL)
                .explanation("Count FlowFiles value: " + countFlowFiles)
                .build());
        } else if ("Duplication".equals(stepName)) {
            final int numCopies = configContext.getProperty(DUPLICATION_STEP, DUPLICATION_GROUP, NUM_COPIES).asInteger();
            results.add(new ConfigVerificationResult.Builder()
                .verificationStepName("Number of Copies Value")
                .outcome(ConfigVerificationResult.Outcome.SUCCESSFUL)
                .explanation("Number of Copies value: " + numCopies)
                .build());
        } else if ("Destination".equals(stepName)) {
            final boolean logContents = configContext.getProperty(DESTINATION_STEP, DESTINATION_GROUP, LOG_FLOWFILE_CONTENTS).asBoolean();
            results.add(new ConfigVerificationResult.Builder()
                .verificationStepName("Log FlowFile Contents Value")
                .outcome(ConfigVerificationResult.Outcome.SUCCESSFUL)
                .explanation("Log FlowFile Contents value: " + logContents)
                .build());
        }
