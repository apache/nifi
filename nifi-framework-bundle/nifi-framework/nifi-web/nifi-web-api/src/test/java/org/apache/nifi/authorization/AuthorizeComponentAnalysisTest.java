...
class AuthorizeComponentAnalysisTest {
    ...
    void testAuthorizeReadOnComponentThenReferences() {
        ...
        AuthorizeComponentAnalysis.authorize(authorizer, authorizableLookup, componentAuthorizable, Map.of(), parameterContext);

        verify(componentDelegate).authorize(eq(authorizer), eq(RequestAction.READ), any());
    }

    @Test
    void testAuthorizeProposedReferencesSkipsComponentRead() {
        AuthorizeComponentAnalysis.authorizeProposedReferences(authorizer, authorizableLookup, componentAuthorizable, Map.of(), parameterContext);
