...
public class TestStandardFunnel {
    ...
    public void testSetFlowFileLimit() {
        {
            final StandardFunnel funnel = getStandardFunnel(1, 100000);
            assertEquals(1, funnel.getMaxConcurrentTasks());
            assertEquals(100, funnel.maxIterations);
        }
        {
            final StandardFunnel funnel = getStandardFunnel(1, 100001);
            assertEquals(1, funnel.getMaxConcurrentTasks());
            assertEquals(101, funnel.maxIterations);
        }
        {
            final StandardFunnel funnel = getStandardFunnel(1, 99999);
            assertEquals(1, funnel.getMaxConcurrentTasks());
            assertEquals(100, funnel.maxIterations);
        }
        {
            final StandardFunnel funnel = getStandardFunnel(1, 0);
            assertEquals(1, funnel.getMaxConcurrentTasks());
            assertEquals(1, funnel.maxIterations);
        }
        {
            final StandardFunnel funnel = getStandardFunnel(1, 1);
            assertEquals(1, funnel.getMaxConcurrentTasks());
            assertEquals(1, funnel.maxIterations);
        }
    }
