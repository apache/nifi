...
@Tags({"sql", "record", "jdbc", "put", "database", "update", "insert", "delete", "table", "tables", "multi"})
@CapabilityDescription("The PutMultiTableDatabaseRecord processor uses a specified RecordReader to input (possibly multiple) records from an incoming flow file into multiple database tables. These records are translated to SQL "
        ...
        + "exist).")
@ReadsAttribute(attribute = PutMultiTableDatabaseRecord.STATEMENT_TYPE_ATTRIBUTE, description = "If 'Use statement.type Attribute' is selected for the Statement Type property, the value of this attribute "
        + "will be used to determine the type of statement (INSERT, UPDATE, DELETE, SQL, etc.) to generate and execute.")
@WritesAttribute(attribute = PutMultiTableDatabaseRecord.PUT_DATABASE_RECORD_ERROR, description = "If an error occurs during processing, the flow file will be routed to failure or retry, and this attribute "
        + "will be populated with the cause of the error.")
@UseCase(description = "Insert records into a database")
public class PutMultiTableDatabaseRecord extends AbstractProcessor {
    ...
    static final PropertyDescriptor AUTO_COMMIT = new PropertyDescriptor.Builder()
            ...
            .required(false)
            .build();
    static final PropertyDescriptor SQL_ONLY = new PropertyDescriptor.Builder()
            .name("Sql only")
            .description("If set to true, the generated sql is routed to the success relation ship.")
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        try {
            ...
            String sql = putToDatabase(context, session, flowFile, connection);
            ...
            if(sql!=null) {
                FlowFile sqlFlowFile = session.write(flowFile, outputStream -> {
    ...
    private String executeDML(final ProcessContext context, final ProcessSession session, final FlowFile flowFile,
        ...
        final int maxBatchSize = context.getProperty(MAX_BATCH_SIZE).evaluateAttributeExpressions(flowFile).asInteger();
        final boolean sqlOnly = context.getProperty(SQL_ONLY).evaluateAttributeExpressions(flowFile).asBoolean();
        final int timeoutMillis = context.getProperty(QUERY_TIMEOUT).evaluateAttributeExpressions().asTimePeriod(TimeUnit.MILLISECONDS).intValue();
...
//        }
        RecordSchema topRecordSchema = recordReader.getSchema();
        Record outerRecord;
        while ((outerRecord = recordReader.nextRecord()) != null) {

            for (RecordField recordField : topRecordSchema.getFields()) {
                //lastPreparedStatement = null;
                final String tableName = recordField.getFieldName();
                final SchemaKey schemaKey = new PutMultiTableDatabaseRecord.SchemaKey(catalog, schemaName, tableName);
                ...
                final List<Record> dataRecords = getDataRecords(outerRecord, recordField);
                ...
                try {
                    for (final Record currentRecord : dataRecords) {
                        ...
                        if (USE_RECORD_PATH.equalsIgnoreCase(explicitStatementType)) {
                            statementType = recordPathOperationType.apply(currentRecord);
                        } else {
                        ...
                        if (preparedSqlAndColumns == null) {
                            ...
                            final String sql = sqlHolder.getSql();
                        ...
                        for (int i = 0; i < fieldIndexes.size(); i++) {
                            ...
                            DataType dataType = dataTypes.get(currentFieldIndex);
                            int fieldSqlType=Integer.MIN_VALUE;
                            final ColumnDescription column = columns.get(columnName);

                            if(dataType.getFieldType()==RecordFieldType.CHOICE){
                                if(column!=null) {
                                    fieldSqlType = column.getDataType();
                                    if (fieldSqlType == -150) {
                                        fieldSqlType = -156;
                                    }
                                }else{
                                    dataType = ((ChoiceDataType)dataType).getPossibleSubTypes().get(0);
                                }
                            }
                            if(fieldSqlType==Integer.MIN_VALUE) {
                                fieldSqlType = DataTypeUtils.getSQLTypeValue(dataType);
                            }
                            int sqlType;
                        ...
                        if (stringBuilder != null) {
                            stringBuilder.append(ps.toString());
            ...
            try {
                ...
                if (fieldSqlType == Types.OTHER && sqlType == Types.VARCHAR) {
                    ...
                    if (sqlType == Types.TIMESTAMP) {
                        if (value != null && value.getClass()==java.lang.Long.class) {
                            Timestamp ts = new Timestamp((long) value);
                            ps.setObject(index, ts, sqlType);
                        } else {
                            ps.setObject(index, value, sqlType);
                        }
                    } else {
    ...
    private List<Record> getDataRecords(final Record outerRecord, RecordField recordField) {
        Object[] value = (Object[])outerRecord.getValue(recordField);
        final List<Record> dataRecords = new ArrayList<>(value.length);
        for(int i=0;i<value.length;i++) {
            dataRecords.add((Record) value[i]);
        }
        return dataRecords;
    }
    ...
    private String putToDatabase(final ProcessContext context, final ProcessSession session, final FlowFile flowFile, final Connection connection) throws Exception {
        ...
        try (final InputStream in = session.read(flowFile)) {
            ...
            if (SQL_TYPE.equalsIgnoreCase(statementType)) {
                ...
                String sql = executeDML(context, session, flowFile, connection, recordReader, statementType, settings);
                return sql;
            }
        }
        return null;
