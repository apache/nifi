...
public class MultipleFlowFileSourceProxy extends PythonProcessorProxy<MultipleFlowFileSource> {
    ...
    public MultipleFlowFileSourceProxy(final String processorType, final Supplier<PythonProcessorBridge> bridgeFactory, final boolean initialize) {
        super(processorType, bridgeFactory, initialize);
    }

    @Override
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        final List<FlowFileSourceResult> results;
        try {
            MultipleFlowFileSource transform = getTransform();
            results = transform.createFlowFiles();
            if (results == null) {
                return;
            }
