...
public abstract class AbstractCouchbaseProcessor extends AbstractProcessor {
    ...
    protected final Collection openCollection(final ProcessContext context, final Bucket bucket) {
        return bucket.collection(context.getProperty(COLLECTION_NAME).evaluateAttributeExpressions().getValue());
    }

    /**
     * Generate a transit url.
     *
     * @param bucket the target collection
     * @return a transit url based on the collection name and the CouchbaseClusterControllerService name
     */
