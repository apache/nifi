...
public class CouchbaseRecordLookupService extends AbstractCouchbaseLookupService implements RecordLookupService {
    ...
    public Optional<Record> lookup(Map<String, Object> coordinates) throws LookupFailureException {
        ...
        final Collection collection = bucket.collection(collectionName);
        ...
        try {
            switch (documentType) {
                ...
                case Binary:
                    inputStream = docId
                            .map(key -> collection.get(docId.get(), GetOptions.getOptions().transcoder(RawBinaryTranscoder.INSTANCE)))
                            .map(doc -> new ByteBufInputStream(doc.contentAs(ByteBuf.class)));
                    break;
