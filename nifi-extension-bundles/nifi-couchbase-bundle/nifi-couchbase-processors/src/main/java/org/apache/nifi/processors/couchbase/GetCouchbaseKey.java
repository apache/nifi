...
public class GetCouchbaseKey extends AbstractCouchbaseProcessor {
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        if (context.getProperty(DOC_ID).isSet()) {
            ...
            session.read(inFile, in -> StreamUtils.fillBuffer(in, content, true));
        ...
        try {
            ...
            final Collection collection = openCollection(context, bucket);
            ...
            switch (documentType) {
                case Json:
                    result = collection.get(docId, GetOptions.getOptions()
                ...
                case Binary:
                    result = collection.get(docId, GetOptions.getOptions().transcoder(RawBinaryTranscoder.INSTANCE));
                    if (result != null) {
                        outputStreamCallback = out -> {
                            // Write to OutputStream without copying any to heap.
                            final ByteBuf byteBuf = result.contentAs(ByteBuf.class);
            ...
            if (putToAttribute) {
                ...
                switch (documentType) {
                    case Json:
                        updatedAttrs.put(putTargetAttr, CouchbaseUtils.getStringContent(result.contentAsObject()));
                        break;
                    case Binary:
                        updatedAttrs.put(putTargetAttr, CouchbaseUtils.getStringContent(result.contentAsBytes()));
                        break;
                }
            } else {
            ...
            updatedAttrs.put(CouchbaseAttributes.Expiry.key(), String.valueOf(result.expiryTime().orElse(null)));
