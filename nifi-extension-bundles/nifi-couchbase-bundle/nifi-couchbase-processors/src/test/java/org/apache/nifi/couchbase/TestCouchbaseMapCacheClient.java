...
public class TestCouchbaseMapCacheClient {
    ...
    public void testGet() throws Exception {
        ...
        properties.put(COLLECTION_NAME, "collectionA");

        final byte[] contents = "value".getBytes(StandardCharsets.UTF_8);
        when(couchbaseService.openBucket(eq("bucketA"))).thenReturn(bucket);
        when(bucket.collection("collectionA")).thenReturn(collection);
        when(getResult.contentAsBytes()).thenReturn(contents);
        when(collection.get(anyString(), any())).thenReturn(getResult);
