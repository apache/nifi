...
public class MockCassandraProcessor extends AbstractProcessor {
    ...
    /**
     * Utility method to get the CassandraSessionProviderService
     * @param context ProcessContext during onTrigger
     * @return CassandraSessionProviderService instance
     */
    public CassandraSessionProviderService getSessionProvider(ProcessContext context) {
        return context.getProperty(CASSANDRA_SESSION_PROVIDER)
                .asControllerService(CassandraSessionProviderService.class);
    }

    @Override
    public void onTrigger(ProcessContext context, ProcessSession session) throws ProcessException {
        CassandraSessionProviderService service = getSessionProvider(context);
        if (service != null) {
            try {
                service.getCassandraSession();
            } catch (Exception e) {
                getLogger().warn("Cassandra session not available, skipping", e);
            }
        }
    }
