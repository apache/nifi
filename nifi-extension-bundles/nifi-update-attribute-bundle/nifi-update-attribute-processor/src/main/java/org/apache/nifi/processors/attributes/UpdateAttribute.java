...
public class UpdateAttribute extends AbstractProcessor implements Searchable {
    ...
    private static final Validator DELETE_PROPERTY_VALIDATOR = new Validator() {
        private final Validator dpvReValidator = StandardValidators.createRegexValidator(0, Integer.MAX_VALUE, true);
        @Override
        public ValidationResult validate(String subject, String input, ValidationContext context) {
            ...
            return dpvReValidator.validate(subject, input, context);
        }
    };
    ...
    private FlowFile evaluateSequential(final ProcessSession session, final ProcessContext context, final Criteria criteria, FlowFile flowfile,
        ...
        FlowFile currentFlowFile = executeActions(session, context, null, defaultActions, flowfile, stateInitialAttributes, stateWorkingAttributes);

        final List<String> matchedRuleNames = new ArrayList<>();
        for (final Rule rule : criteria.getRules()) {
            if (evaluateRule(context, rule, currentFlowFile, stateInitialAttributes)) {
                if (debugEnabled) {
                    getLogger().debug("{} sequential rule '{}' matched; applying actions immediately.", this, rule.getName());
                }
                currentFlowFile = executeActions(session, context, List.of(rule), Map.of(), currentFlowFile, stateInitialAttributes, stateWorkingAttributes);
