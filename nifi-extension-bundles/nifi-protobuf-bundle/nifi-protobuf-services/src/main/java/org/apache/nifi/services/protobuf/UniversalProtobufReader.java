...
@CapabilityDescription("Parses Protocol Buffers messages from binary format into NiFi Records. " +
    "Supports multiple schema access strategies including inline schema text, schema registry lookup, " +
    "and schema reference readers. The message name can be determined statically via configuration " +
    "or dynamically using a Message Name Resolver service.")

public class UniversalProtobufReader extends SchemaRegistryService implements RecordReaderFactory {
    ...
    public void onEnabled(final ConfigurationContext context) {
        ...
        this.schemaAccess = context.getProperty(SCHEMA_ACCESS_STRATEGY).getValue();
    ...
    public RecordReader createRecordReader(final Map<String, String> variables, final InputStream in, final long inputLength, final ComponentLog logger) throws IOException, SchemaNotFoundException {
        ...
        if (SCHEMA_TEXT_PROPERTY.getValue().equals(schemaAccess)) {
            final SchemaDefinition schemaDefinition = createSchemaDefinitionFromText(variables);
            if (logger.isDebugEnabled()) {
                logger.debug("For {} found schema text {}", variables, schemaDefinition.getText());
            }
            return createProtobufRecordReader(variables, in, schemaDefinition);
        } else if (SCHEMA_NAME_PROPERTY.getValue().equals(schemaAccess)) {
            final SchemaDefinition schemaDefinition = createSchemaDefinitionFromRegistry(variables);
            return createProtobufRecordReader(variables, in, schemaDefinition);
        } else if (SCHEMA_REFERENCE_READER_PROPERTY.getValue().equals(schemaAccess)) {
    ...
    private void setupMessageNameResolver(final ConfigurationContext context) {
        ...
        switch (messageNameResolverStrategyName) {
            case MESSAGE_NAME_PROPERTY -> {
                messageNameResolver = new PropertyMessageNameResolver(context);
            }
            case MESSAGE_NAME_RESOLVER_SERVICE -> {
                messageNameResolver = context.getProperty(MESSAGE_NAME_RESOLVER_CONTROLLER_SERVICE)
                    .asControllerService(MessageNameResolverService.class);
            }
            default -> {
                throw new IllegalStateException("Unsupported Message Name Resolver Strategy: " + messageNameResolverStrategyName);
            }
        }
    ...
    private SchemaDefinition createSchemaDefinitionFromText(final Map<String, String> variables) throws SchemaNotFoundException {
        ...
        final String sha256hex = DigestUtils.sha256Hex(schemaTextString);
