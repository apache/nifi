...
/**
 * A fallback CQL {@code date} codec that accepts any date-compatible value {@code DataTypeUtils} recognizes,
 * including a {@link String} parsed with the fixed ISO {@code yyyy-MM-dd} pattern (what a RecordPath
 * {@code format()} primary key override produces). Declared against {@link Object} so the registry treats it
 * as a fallback behind the built-in {@link LocalDate} codec rather than rejecting it as a duplicate.
 */
