...
@CapabilityDescription("Consumes messages from Apache Kafka Consumer API. "
        ...
        + "When configured for 'Share Group' the Processor uses Kafka share groups (KIP-932, requires Kafka 4.1+ brokers configured for share-group operation; "
...
public class ConsumeKafka extends AbstractProcessor implements VerifiableProcessor, BacklogReportingProcessor {
    ...
    static final PropertyDescriptor GROUP_TYPE = new PropertyDescriptor.Builder()
            ...
            .description("Selects the Kafka consumer group model. Choose 'Consumer Group' for the classic offset-committed model used by NiFi historically. "
                    + "Choose 'Share Group' to use Kafka share groups (KIP-932), which require Kafka 4.1+ brokers configured for share-group operation; "
                    + "Kafka 4.2+ is recommended because share groups are Generally Available. Share groups "
    ...
    static final PropertyDescriptor TOPIC_FORMAT = new PropertyDescriptor.Builder()
            ...
            .description("Specifies whether the Topics provided are a comma separated list of names or a single regular expression. "
                    + "Pattern subscription is not available for share groups.")
            .required(true)
    ...
    public Optional<Backlog> getBacklog(final ProcessContext context) throws BacklogReportingException {
        final GroupType configuredGroupType = context.getProperty(GROUP_TYPE).asAllowableValue(GroupType.class);
        if (configuredGroupType == GroupType.SHARE) {
