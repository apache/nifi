...
import static org.apache.nifi.kafka.shared.component.KafkaClientComponent.AWS_CREDENTIALS_PROVIDER_SERVICE;
    ...
    private void setSecurityProperties(final Map<String, Object> properties, final PropertyContext context) {
        ...
        if (SecurityProtocol.SASL_PLAINTEXT == securityProtocol || SecurityProtocol.SASL_SSL == securityProtocol) {
            ...
            if (saslMechanism == SaslMechanism.GSSAPI && isCustomKerberosLoginFound()) {
                ...
                final PropertyValue credentialsProviderService = context.getProperty(AWS_CREDENTIALS_PROVIDER_SERVICE);
                if (credentialsProviderService != null && credentialsProviderService.isSet()) {
                    properties.put(SASL_CLIENT_CALLBACK_HANDLER_CLASS.getProperty(), AwsMskKafkaProperties.NIFI_AWS_MSK_CALLBACK_HANDLER_CLASS);
