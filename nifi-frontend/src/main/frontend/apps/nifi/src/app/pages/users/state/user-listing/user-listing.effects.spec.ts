...
describe('UserListingEffects', () => {
    ...
    async function setup({ userListingState = initialState }: SetupOptions = {}) {
        await TestBed.configureTestingModule({
            providers: [
                ...
                { provide: ErrorHelper, useValue: { handleLoadingError: vi.fn(), getErrorString: vi.fn() } },
                { provide: MatDialog, useValue: { open: vi.fn(), closeAll: vi.fn() } },
                { provide: Router, useValue: { navigate: vi.fn() } },
                { provide: Client, useValue: { getRevision: vi.fn() } },
                { provide: NiFiCommon, useValue: { isEmpty: vi.fn() } }
            ]
        }).compileComponents();
    ...
    describe('Update User Group', () => {
        it('should update user group successfully', async () => {
            ...
            vi.spyOn(usersService, 'updateUserGroup').mockReturnValueOnce(of(mockResponse) as never);
        ...
        it('should handle update user group error', async () => {
            ...
            const error = new HttpErrorResponse({ status: 500 });
            vi.spyOn(usersService, 'updateUserGroup').mockReturnValueOnce(throwError(() => error) as never);
            vi.spyOn(errorHelper, 'getErrorString').mockReturnValueOnce('Update failed');
        ...
        it('should handle multiple concurrent updateUserGroup actions', async () => {
            ...
            const mockUpdateUserGroup = vi.spyOn(usersService, 'updateUserGroup');
            mockUpdateUserGroup
    ...
    describe('Update User Success', () => {
        ...
        it('should dispatch updateUserComplete when userGroupUpdate has empty added and removed', async () => {
            ...
            vi.spyOn(nifiCommon, 'isEmpty').mockReturnValue(true);
        ...
        it('should dispatch updateUserGroup actions for added user groups', async () => {
            ...
            vi.spyOn(nifiCommon, 'isEmpty').mockImplementation((arr: any) => !arr || arr.length === 0);
            vi.spyOn(client, 'getRevision').mockReturnValue({ version: 1 });
        ...
        it('should dispatch updateUserGroup actions for removed user groups', async () => {
            ...
            vi.spyOn(nifiCommon, 'isEmpty').mockImplementation((arr: any) => !arr || arr.length === 0);
            vi.spyOn(client, 'getRevision').mockReturnValue({ version: 2 });
