...
export const bulkReplayStatusReducer = createReducer(
    ...
    on(loadJobSummaryDeleted, (state, { jobId }) => {
        const jobs = state.jobs.filter((job) => job.jobId !== jobId);
        const { [jobId]: _removed, ...jobItems } = state.jobItems;
        return { ...state, jobs, jobItems };
    }),
