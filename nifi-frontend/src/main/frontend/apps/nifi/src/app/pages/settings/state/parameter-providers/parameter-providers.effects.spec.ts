...
describe('ParameterProvidersEffects', () => {
    ...
    describe('loadParameterProviders$', () => {
        ...
        it('should handle error when loading parameter providers fails', async () => {
            const mockError = new HttpErrorResponse({ status: 500, statusText: 'Server Error' });
            const mockErrorAction = { type: 'ERROR_ACTION' };

            mockParameterProviderService.getParameterProviders.mockReturnValue(throwError(() => mockError));
            mockErrorHelper.handleLoadingError.mockReturnValue(mockErrorAction);

            actions$.next(ParameterProviderActions.loadParameterProviders());

            const action = await firstValueFrom(effects.loadParameterProviders$);

            expect(action).toEqual(mockErrorAction);
            expect(mockErrorHelper.handleLoadingError).toHaveBeenCalledWith(expect.any(String), mockError);
        });
    });
