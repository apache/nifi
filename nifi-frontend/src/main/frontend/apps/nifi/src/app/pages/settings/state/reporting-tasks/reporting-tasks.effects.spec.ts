...
describe('ReportingTasksEffects', () => {
    ...
    describe('loadReportingTasks$', () => {
        ...
        it('should handle error when loading reporting tasks fails', async () => {
            ...
            expect(mockErrorHelper.handleLoadingError).toHaveBeenCalledWith('pending', errorResponse);
            expect(result).toEqual({ type: '[Error] Loading Error' });
        });
    });
