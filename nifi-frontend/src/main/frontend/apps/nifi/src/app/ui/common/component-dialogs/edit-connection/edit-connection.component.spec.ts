...
import { EditConnectionDialogRequest } from '../../../../state/shared';
...
describe('EditConnectionComponent', () => {
    ...
    async function setup(options: { dialogRequest?: EditConnectionDialogRequest } = {}) {
    ...
    describe('Submit form', () => {
        it('should emit editConnection with payload when submitting form', async () => {
            const connection = createMockConnection();
            const dialogRequest = createMockDialogRequest(connection);
            const { component } = await setup({ dialogRequest });

            const editSpy = vi.fn();
            component.editConnection.subscribe(editSpy);
