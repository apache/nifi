...
public final class SslContextFactory {
    ...
    // TODO: Add translations from "NONE/0, WANT/1, NEED/2"
    // TODO: Add helper methods for name/parsing
    ...
    public static Tuple<SSLContext, TrustManager[]> createSslContextWithTrustManagers(
        ...
        // This can be empty if the context is only used for "remote endpoint authentication" TLS
        KeyManagerFactory keyManagerFactory = createKeyManagerFactory(keystore, keystorePasswd, keyPasswd, keystoreType);
    ...
    private static KeyManagerFactory createKeyManagerFactory(String keystore, char[] keystorePasswd, char[] keyPasswd,
                                                             String keystoreType)
            throws KeyStoreException, IOException, NoSuchAlgorithmException, CertificateException, UnrecoverableKeyException {
        final KeyManagerFactory keyManagerFactory = KeyManagerFactory.getInstance(KeyManagerFactory.getDefaultAlgorithm());

        if (StringUtils.isEmpty(keystore) || !Arrays.stream(KeystoreType.values()).map(kt -> kt.getName().toUpperCase()).collect(Collectors.toList()).contains(keystoreType.toUpperCase())) {
            logger.warn("Cannot create key manager factory with missing keystore or keystore type");
            return null;
        }
    ...
    public static Tuple<SSLContext, TrustManager[]> createTrustSslContextWithTrustManagers(
        ...
        if (keystore == null) {
            ...
            sslContextWithTrustManagers = createSslContextWithTrustManagers(keystore, keystorePasswd, keyPasswd,
                    keystoreType, truststore, truststorePasswd, truststoreType, clientAuth, protocol);
        }
        return new Tuple<>(sslContextWithTrustManagers.getKey(), sslContextWithTrustManagers.getValue());
    }

    public static Tuple<SSLContext, TrustManager[]> createSslContextFromControllerService(SSLConfig sslConfig)
            throws CertificateException, UnrecoverableKeyException, NoSuchAlgorithmException, KeyStoreException,
            KeyManagementException, IOException {
        return createSslContextWithTrustManagers(
                sslConfig.getKeystorePath(),
                sslConfig.getKeystorePasswordAsChars(),
                sslConfig.getKeyOrKeystorePasswordAsChars(),
                sslConfig.getKeystoreTypeAsString(),
                sslConfig.getTruststorePath(),
                sslConfig.getTruststorePasswordAsChars(),
                sslConfig.getTruststoreTypeAsString(),
