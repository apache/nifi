...
public class AzureKeyVaultKeySensitivePropertyProvider extends AbstractSensitivePropertyProvider {
    ...
    private void initializeClient() {
        ...
        try {
            ...
            final String msg = "Valid Azure Key Vault configuration/credentials are required to initialize Key Vault Client";
    ...
    private void validate() throws SensitivePropertyProtectionException {
        if (client == null) {
            final String msg = "The Azure Client failed to open, cannot validate key";
            throw new SensitivePropertyProtectionException(msg);
        }

        if (StringUtils.isBlank(keyId)) {
            final String msg = "The Azure Key Vault key provided is blank";
            throw new SensitivePropertyProtectionException(msg);
        }
        ...
        try {
            ...
            if (!keyProps.isEnabled()) {
                throw new SensitivePropertyProtectionException("The Azure Key Vault key is not enabled");
            }
            ...
            if (!(keyOps.contains(KeyOperation.ENCRYPT) && keyOps.contains(KeyOperation.DECRYPT))) {
                throw new SensitivePropertyProtectionException("The Azure Key Vault key cannot be used to encrypt/decrypt");
            }
        } catch (final ResourceNotFoundException e) {
            final String msg = "Azure Key Vault key was not found";
            throw new SensitivePropertyProtectionException(msg);
        } catch (final RuntimeException e) {
            final String msg = "Azure Key Vault client incurred a runtime exception";
            throw new SensitivePropertyProtectionException(msg);
        }
    }
    ...
    private void checkAndInitializeClient() throws SensitivePropertyProtectionException {
        if (client == null) {
            try {
                ...
                throw new SensitivePropertyProtectionException("Error initializing the Azure Key Vault Client", e);
            }
        }
    }
