    ...
    public static void main(final String[] arguments) {
        if (arguments.length == 2) {
            ...
            if (username.length() < MINIMUM_USERNAME_LENGTH) {
                System.err.printf("Username length less than required [%d]%n", MINIMUM_USERNAME_LENGTH);
            } else if (password.length() < MINIMUM_PASSWORD_LENGTH) {
                System.err.printf("Password length less than required [%d]%n", MINIMUM_PASSWORD_LENGTH);
            } else {
