...
public class CompositeClientTest extends EmbeddedQuestDbTest {
    ...
    public void testSingleErrorDuringExecution() throws DatabaseException {
        Mockito.doThrow(new DatabaseException("Test")).doNothing().when(client).execute(SELECT_QUERY);
    ...
    public void testConstantErrorDuringExecution() throws DatabaseException {
        Mockito.doThrow(new DatabaseException("Test")).when(client).execute(SELECT_QUERY);
    ...
    public void testWhenConditionDisallowsRun() throws DatabaseException {
        Mockito.when(condition.check()).thenReturn(false);
    ...
    public void testWhenConditionDisallowsRunForTheFirstAttempt() throws DatabaseException {
        Mockito.when(condition.check()).thenReturn(false).thenReturn(true);
    ...
    private RetryingClient getTestSubject() {
        ...
        lockedClientSpy = Mockito.spy(lockedClient);
    ...
    private void assertWrappedClientIsCalled(final int times) throws DatabaseException {
        Mockito.verify(client, Mockito.times(times)).execute(SELECT_QUERY);
    }

    private void assertFallbackIsNotCalled() throws DatabaseException {
        Mockito.verify(fallback, Mockito.never()).execute(Mockito.anyString());
    }

    private void assertFallbackIsCalled() throws DatabaseException {
        Mockito.verify(fallback, Mockito.times(1)).execute(SELECT_QUERY);
    }

    private void assertErrorActionIsCalled(final int times) {
        Mockito.verify(errorAction, Mockito.times(times)).accept(Mockito.any(Integer.class), Mockito.any(Exception.class));
    }

    private void assertErrorActionIsNotCalled() {
        Mockito.verify(errorAction, Mockito.never()).accept(Mockito.any(Integer.class), Mockito.any(Exception.class));
    }
}
