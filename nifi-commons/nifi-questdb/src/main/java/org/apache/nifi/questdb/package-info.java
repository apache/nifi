 ...
 *
 * <p>
 *     For ease of use, the bundle supports "mapping" between "entries" (representation of data within NiFi) and "rows" (representation of
 *     data within QuestDB) for both ways. The mapping can be an "insert mapping" (<code>InsertMapping</code>) which is used for
 *     putting data into the database or a "request mapping" (<code>RequestMapping</code>) which is applied during querying datatables.
 *     Both can be created by the accompanying builder. In order to use, they must be wrapped by <code>InsertRowDataSource#forMapping</code>
 *     and <code>QueryResultProcessor#forMapping</code> accordingly.
 * </p>
 *
