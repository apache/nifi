...
public class NiFiStateless {
    ...
    private static final String STATELESS_CORE_NAR_ID = "nifi-stateless-nar";
    ...
    public static void main(final String[] args) throws IOException, ClassNotFoundException, NoSuchMethodException, InvocationTargetException, IllegalAccessException {
        ...
        File statelessCoreWorkingDirectory = null;
        ...
        for (final File narFile : narFiles) {
            ...
            if (isStatelessCoreNar(narFile)) {
                statelessCoreWorkingDirectory = unpackedDirectory;
            }
        }
        ...
        if (statelessCoreWorkingDirectory == null) {
            throw new FileNotFoundException("Could not find NiFi Stateless core NAR in the lib directory <" + libDir + ">");
        }
    ...
    private static boolean isStatelessCoreNar(final File file) throws IOException {
        final String filename = file.getName();
        if (filename.startsWith("nifi-stateless") && filename.endsWith(".nar")) {
            try (final JarFile jarFile = new JarFile(file)) {
                ...
                return STATELESS_CORE_NAR_ID.equals(narId);
            }
        }
    ...
    private static void printUsage() {
        ...
        System.out.println("    3) " + RUN_YARN_SERVICE_FROM_REGISTRY + " ./lib ./work http://127.0.0.1:8088 nifi-stateless:latest kafka-to-solr 3 --file kafka-to-solr.json");
