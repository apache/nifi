...
public class PrimaryRoleAssignmentExceptionMapper implements ExceptionMapper<PrimaryRoleAssignmentException> {
    ...
    @Override
    public Response toResponse(PrimaryRoleAssignmentException ex) {
        // log the error
        logger.info(String.format("Failed to assign primary role due to %s. Returning %s response.", ex, Response.Status.CONFLICT));

        if (logger.isDebugEnabled()) {
            logger.debug(StringUtils.EMPTY, ex);
        }

        return Response.status(Response.Status.CONFLICT).entity(ex.getMessage()).type("text/plain").build();
    }
