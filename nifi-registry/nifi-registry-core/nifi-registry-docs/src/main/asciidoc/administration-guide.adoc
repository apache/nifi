...
If you are using database for storing buckets and flows (as described in the 'Database Properties' section above), you can store the users, groups and policies in the same database as well. 

**Note:** This UserGroupProvider is available from version 0.8.0.


The DatabaseUserGroupProvider takes the database connection information from `nifi-registry.properties` file, so in the `authorizers.xml` file you don't need to give any additional information about the DB (full example of `authorizers.xml` file for using database implementation desribed in `DatabaseAccessPolicyProvider` section).

The users and groups are stored in the DB in the tables:

* **UGP_USER -** Stores all the users by: user-ID (IDENTIFIER) and user-name (IDENTITY).
* **UGP_GROUP -** Stores all the groups by: group-ID (IDENTIFIER) and group-name (IDENTITY).
* **UGP_USER_GROUP -** Mapping between users and groups, so each row contains the fields: USER_IDENTIFIER and GROUP_IDENTIFIER, represents that the user is contained in the group.
...
Like `DatabaseUserGroupProvider` described above, this type of PolicyProvider takes the database connection details from `nifi-registry.properties` file. 
The policies are stored in the tables:


* **APP_POLICY -** Stores a policy object contains: Identifier, Resource and Action (for more info about policies, Please see **Access Policies** section below).
* **APP_POLICY_GROUP -** Mapping between a gruop and a policy that it has. Contains: PolicyIdentifier and GroupIdentifier.
* **APP_POLICY_USER -** Mapping between a user and a policy that it has. Contains: PolicyIdentifier and UserIdentifier.

Here is an example of an implementation of Database access-policy provider:
...
<authorizers>
    <userGroupProvider>
        ...
        <property name="Initial User Identity 1">AdminUserName</property>
    </userGroupProvider>
    <accessPolicyProvider>
        ...
        <property name="Initial Admin Identity">AdminUserName</property>
        <property name="NiFi Identity 1"></property>
    </accessPolicyProvider>
    <authorizer>
        <identifier>managed-authorizer</identifier>
