...
public class PropertyAddedRebaseHandler implements RebaseHandler {

    private static final Logger logger = LoggerFactory.getLogger(PropertyAddedRebaseHandler.class);

    @Override
    public DifferenceType getSupportedType() {
        return DifferenceType.PROPERTY_ADDED;
    }
    ...
    public RebaseAnalysis.ClassifiedDifference classify(final FlowDifference localDifference, final Set<FlowDifference> upstreamDifferences,
        ...
        for (final FlowDifference upstreamDifference : upstreamDifferences) {
            if ((upstreamDifference.getDifferenceType() == DifferenceType.PROPERTY_ADDED
                        || upstreamDifference.getDifferenceType() == DifferenceType.PROPERTY_CHANGED)
    ...
    public void apply(final FlowDifference localDifference, final VersionedProcessGroup mergedFlow) {
        ...
        if (component == null) {
            logger.warn("Unable to apply property addition: configurable component [{}] not found in merged flow", componentIdentifier);
            return;
        }
