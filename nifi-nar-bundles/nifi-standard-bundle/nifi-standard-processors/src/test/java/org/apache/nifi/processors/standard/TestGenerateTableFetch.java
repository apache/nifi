...
public class TestGenerateTableFetch {
    ...
    public void testAddedRowsRightBounded() throws ClassNotFoundException, SQLException, InitializationException, IOException {
        ...
        runner.setIncomingConnection(false);
        runner.setProperty(GenerateTableFetch.MAX_VALUE_COLUMN_NAMES, "ID");
        runner.setProperty(GenerateTableFetch.RIGHT_BOUND_WHERE, "true");
    ...
    @Test
    public void testAddedRowsRightBoundedMaximumRowCount() throws ClassNotFoundException, SQLException, InitializationException, IOException {
        ...
        int i=0;
        stmt.execute("create table TEST_QUERY_DB_TABLE (id integer not null, name varchar(100), scale float, created_on timestamp, bignum bigint default 0)");
        stmt.execute("insert into TEST_QUERY_DB_TABLE (id, name, scale, created_on) VALUES ("+i+++", 'Joe Smith', 1.0, '1962-09-23 03:23:34.234')");
        stmt.execute("insert into TEST_QUERY_DB_TABLE (id, name, scale, created_on) VALUES ("+i+++", 'Carrie Jones', 5.0, '2000-01-01 03:23:34.234')");
        stmt.execute("insert into TEST_QUERY_DB_TABLE (id, name, scale, created_on) VALUES ("+i+++", 'Carrie Jones', 2.0, '2000-01-01 03:23:34.234')");
        stmt.execute("insert into TEST_QUERY_DB_TABLE (id, name, scale, created_on) VALUES ("+i+++", 'Carrie Jones', 2.0, '2000-01-02 03:23:34.234')");
        stmt.execute("insert into TEST_QUERY_DB_TABLE (id, name, scale, created_on) VALUES ("+i+++", 'Carrie Jones', 2.0, '2000-01-02 03:23:34.234')");
        stmt.execute("insert into TEST_QUERY_DB_TABLE (id, name, scale, created_on) VALUES ("+i+++", 'Carrie Jones', 2.0, '2000-01-03 03:23:34.234')");
        stmt.execute("insert into TEST_QUERY_DB_TABLE (id, name, scale, created_on) VALUES ("+i+++", 'Carrie Jones', 2.0, '2000-01-04 03:23:34.234')");

        runner.setProperty(GenerateTableFetch.TABLE_NAME, "TEST_QUERY_DB_TABLE");
        runner.setIncomingConnection(false);
        runner.setProperty(GenerateTableFetch.MAX_VALUE_COLUMN_NAMES, "created_on");
        runner.setProperty(GenerateTableFetch.RIGHT_BOUND_WHERE, "true");
        runner.setProperty(GenerateTableFetch.MAXIMUM_ROWS, "2");
        ...
        MockFlowFile flowFile = runner.getFlowFilesForRelationship(REL_SUCCESS).get(0);
        String query = new String(flowFile.toByteArray());
        assertEquals("SELECT * FROM TEST_QUERY_DB_TABLE WHERE ID <= 0 ORDER BY ID FETCH NEXT 1 ROWS ONLY", query);
        ResultSet resultSet = stmt.executeQuery(query);
        // Should be three records
        assertTrue(resultSet.next());
        assertTrue(resultSet.next());
