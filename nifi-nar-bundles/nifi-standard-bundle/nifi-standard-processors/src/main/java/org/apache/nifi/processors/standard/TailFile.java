...
public class TailFile extends AbstractProcessor {
    ...
    static final PropertyDescriptor MAX_BUFFER_LENGTH = new Builder()
        ...
        .description("When using the <Line Start Regex>, there may be situations in which the data in the file being tailed never matches the Regular Expression. This would result in the processor " +
    ...
    private volatile Pattern lineStartRegex;
    ...
    protected List<PropertyDescriptor> getSupportedPropertyDescriptors() {
        ...
        properties.add(LINE_START_REGEX);
    ...
    public void compileRegex(final ProcessContext context) {
        final String regex = context.getProperty(LINE_START_REGEX).getValue();
        lineStartRegex = (regex == null) ? null : Pattern.compile(regex);
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        // If a Line Start Regex is being used and data is buffered, the Position that has been stored in the state will
        // not be accurate. To address this, we call cleanup(), which will handle updating the state to the correct values for us.
        if (lineStartRegex != null && linesBuffer.size() > 0) {
            cleanup(context);
        }
    }
    ...
    private void flushByteArrayOutputStream(final ByteArrayOutputStream baos, final OutputStream out, final Checksum checksum, final boolean ignoreRegex) throws IOException {
        ...
        for (final String line : lines) {
            final boolean startsWithRegex = lineStartRegex.matcher(line).lookingAt();
