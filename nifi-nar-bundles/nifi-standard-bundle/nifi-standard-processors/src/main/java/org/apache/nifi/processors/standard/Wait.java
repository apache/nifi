...
@CapabilityDescription("Routes incoming FlowFiles to the 'wait' relationship until a matching release signal "
        ...
        + "At this point, a waiting FlowFile is routed to the 'success' relationship, with attributes copied from the FlowFile that produced "
        + "the release signal from the Notify processor.  The release signal entry is then removed from the cache. "
        + "Waiting FlowFiles will be routed to 'expired' if they exceed the Expiration Duration. "

        + "If you need to wait more than one signal, you can specify desired count via 'Target Signal Count' property. "
        + "This is particularly useful with processors those split a source flow file into multiple fragments, such as SplitText. "
        + "In order to wait all fragments to be processed, connect 'original' relationship to Wait processor, and 'splits' relationship to "
        + "corresponding Notify processor from a split processor. "
        + "Then configure both Notify and Wait processors to use 'fragment.identifier' attribute as 'Release Signal Identifier', "
        + " by specifying Attribute Expression Language (EL) as '${fragment.identifier}'. "
        + "Similarly, specify an EL '${fragment.count}' at 'Target Signal Count' in Wait processor."
)
@WritesAttributes({
        ...
        @WritesAttribute(attribute = "wait.counter.<counterName>", description = "If a signal exists when processor runs, "
...
public class Wait extends AbstractProcessor {
    ...
    public static final PropertyDescriptor TARGET_SIGNAL_COUNT = new PropertyDescriptor.Builder()
            ...
            .description("A value, or the results of an Attribute Expression Language statement, which will " +
                    ...
                    "This processor checks whether the target count is reached to this number. " +
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        try {
            ...
            if (!reachedToTargetCount) {
                if (logger.isDebugEnabled()) {
                    logger.debug("Release signal count {} doesn't reach to {} yet for {} on FlowFile {}",
