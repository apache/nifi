...
public class DeleteFile extends AbstractProcessor {
    ...
    private final static Set<Relationship> relationships = Collections.unmodifiableSet(new HashSet<>(
            Arrays.asList(REL_SUCCESS, REL_NOT_FOUND, REL_FAILURE)
    ));
    ...
    private final static List<PropertyDescriptor> properties = Collections.unmodifiableList(
            Arrays.asList(DIRECTORY_PATH, FILENAME)
    );
    ...
    public void onTrigger(ProcessContext context, ProcessSession session) throws ProcessException {
        ...
        try {
            ...
            if (!directoryPath.equals(filePath.getParent())) {
                final String errorMessage = String.format("Attempting to delete file at path '%s' which is not a direct child of the directory '%s'", filePath, directoryPath);
            ...
            final String transitUri = String.format("file://%s", filePath);
            ...
            final String errorMessage = String.format("Failed to delete file '%s' in directory '%s'", filename, directoryPathProperty);
