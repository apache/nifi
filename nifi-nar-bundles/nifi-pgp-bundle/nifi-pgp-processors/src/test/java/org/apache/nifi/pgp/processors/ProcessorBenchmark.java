...
class ProcessorBenchmark {
    static List<BenchmarkMeasurement> bench(String title, TestRunner runner, Relationship success,
                                            BenchmarkConfiguration params, BenchmarkConfigurator config)
        ...
        for (int i = 0; i < benchmarkRuns + warmUpRuns; i++) {
            for (String paramsTitle : params.get().keySet()) {
                ...
                config.setRunner(runner, paramsMap);
    ...
    static void run(String title, TestRunner runner, Relationship success, BenchmarkConfiguration params, BenchmarkConfigurator forward) throws IOException, InterruptedException {
        report(bench(title, runner, success, params, forward));
    }
    ...
    interface BenchmarkConfigurator {
        void setRunner(TestRunner runner, Map<PropertyDescriptor, String> config);
    }
