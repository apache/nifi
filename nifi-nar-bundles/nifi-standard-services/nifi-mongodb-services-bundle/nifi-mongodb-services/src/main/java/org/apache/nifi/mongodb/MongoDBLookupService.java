...
@CapabilityDescription(
    ...
    "The query is limited to the first result (findOne in the Mongo documentation). If no \"lookup key\" is specified " +
    "then the entire Mongo result document minus the _id field will be returned as a record and merged."
)
public class MongoDBLookupService extends MongoDBControllerService implements LookupService<Object> {

    public static final PropertyDescriptor LOOKUP_KEY = new PropertyDescriptor.Builder()
            .name("mongo-lookup-key")
            .displayName("Lookup Key")
            .description("The key from the result to return")
    ...
    static {
        ...
        lookupDescriptors.add(LOOKUP_KEY);
    }
    ...
    public Optional<Object> lookup(Map<String, String> coordinates) throws LookupFailureException {
        ...
        try {
            ...
            if (lookupKey != null && !lookupKey.equals("")) {
                return Optional.ofNullable(result.get(lookupKey));
            } else {
    ...
    public void onEnabled(final ConfigurationContext context) throws InitializationException, IOException, InterruptedException {
        this.lookupKey = context.getProperty(LOOKUP_KEY).getValue();
