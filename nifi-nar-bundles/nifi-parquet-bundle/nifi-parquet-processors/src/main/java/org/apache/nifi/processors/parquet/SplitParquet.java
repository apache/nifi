...
import static org.apache.nifi.parquet.utils.ParquetAttribute.RECORD_COUNT;
import static org.apache.nifi.parquet.utils.ParquetAttribute.RECORD_OFFSET;
...
@SideEffectFree
public class SplitParquet extends AbstractProcessor {
    ...
    public void onTrigger(ProcessContext context, ProcessSession session) throws ProcessException {
        ...
        final long recordOffset = Optional.ofNullable(original.getAttribute(RECORD_OFFSET))
                .map(Long::valueOf)
                .orElse(0L);

        final long recordCount = Optional.ofNullable(original.getAttribute(RECORD_COUNT))
                .map(Long::valueOf)
        ...
        for (long currentPartition = 0; currentPartition < numberOfPartitions; currentPartition++) {
            ...
            results.add(
                    session.putAllAttributes(
                            outputFlowFile,
                            Map.of(
                                    RECORD_OFFSET, Long.toString(recordOffset + addedOffset),
                                    RECORD_COUNT, Long.toString(Math.min(partitionSize, recordCount - addedOffset))
                            )
                    )
            );
        }
