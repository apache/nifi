...
@CapabilityDescription("Writes the contents of a FlowFile to Cosmos Document DB with Core SQL API. For CosmosDB with Mongo API, use PutMongo instead.")
@SystemResourceConsideration(resource = SystemResource.MEMORY)
public class PutCosmosDocument extends AbstractCosmosDocumentProcessor {
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        try {
            ...
            // parse conten into CosmosItemProperites
            doc = new CosmosItemProperties(new String(content, charset));
            ...
            if(doc.has("id")) {
                ...
                if(!(val instanceof String)) {
                    ...
                    doc.setId(doc.get("id").toString());
                }
            } else {
                doc.setId(flowFile.getAttribute("uuid"));
            }
            ...
            logger.error("Failed to upsertDocument {} into CosmosDB due to {}", new Object[] {flowFile, e}, e);
            if(doc != null){
                logger.error("Failed doc content : " + doc.toJson());
            }
