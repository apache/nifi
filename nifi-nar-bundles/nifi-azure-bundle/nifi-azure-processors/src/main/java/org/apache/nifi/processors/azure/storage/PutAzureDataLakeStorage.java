...
public class PutAzureDataLakeStorage extends AbstractProcessor {
    ...
    public static final PropertyDescriptor CLIENT_ID = new PropertyDescriptor.Builder().name("clientid").displayName("Client Id")
            .description("ClientId the client ID (GUID) of the client web app obtained from Azure Active Directory configuration").addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
            .expressionLanguageSupported(ExpressionLanguageScope.FLOWFILE_ATTRIBUTES).required(true).build();

    public static final PropertyDescriptor CLIENT_SECRET = new PropertyDescriptor.Builder().name("clientsecret").displayName("Client Secret")
            .description("ClientSecret the secret key of the Azure client web app").addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
            .expressionLanguageSupported(ExpressionLanguageScope.FLOWFILE_ATTRIBUTES).required(true).build();

    public static final PropertyDescriptor AUTH_ENDPOINT = new PropertyDescriptor.Builder().name("authendpoint").displayName("Auth EndPoint")
            .description("OAuth 2.0 token endpoint associated with the user's directory (obtain from Active Directory configuration)").addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
            .expressionLanguageSupported(ExpressionLanguageScope.FLOWFILE_ATTRIBUTES).required(true).build();

    public static final PropertyDescriptor ACCOUNT_FQDN = new PropertyDescriptor.Builder().name("accountfqdn").displayName("Account FQDN")
            .description("Fully qualified domain name of the account.For example, <<datalakestroageaccount>>.azuredatalakestore.net").addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
            .expressionLanguageSupported(ExpressionLanguageScope.FLOWFILE_ATTRIBUTES).required(true).build();

    public static final PropertyDescriptor FOLDER_PATH = new PropertyDescriptor.Builder().name("path").displayName("Path").description("Folder path in Data Lake Storage where file will be copied ")
            .addValidator(StandardValidators.NON_EMPTY_VALIDATOR).expressionLanguageSupported(ExpressionLanguageScope.FLOWFILE_ATTRIBUTES).required(true).build();

    public static final PropertyDescriptor FILE_NAME = new PropertyDescriptor.Builder().name("file").displayName("File")
            .description("File name with extension to be copied in Azure Data Lake Storage ").addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
            .expressionLanguageSupported(ExpressionLanguageScope.FLOWFILE_ATTRIBUTES).required(true).build();
