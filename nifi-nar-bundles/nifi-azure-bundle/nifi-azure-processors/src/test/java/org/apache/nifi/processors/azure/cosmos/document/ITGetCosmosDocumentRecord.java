...
public class ITGetCosmosDocumentRecord extends ITAbstractCosmosDocument {
    ...
    protected Class<? extends Processor> getProcessorClass() {
        return GetCosmosDocumentRecord.class;
    }
    @Before
    public void setupTest() throws Exception
    {
        ...
        runner.setProperty(GetCosmosDocumentRecord.WRITER_FACTORY, "writer");
    }
    @After
    ...
    public void testReadRecords() {
        ...
        List<MockFlowFile> flowFiles = runner.getFlowFilesForRelationship(AbstractCosmosDocumentProcessor.REL_SUCCESS);
    ...
    public void testReadTop5Recrods() {
        JsonParser parser = new JsonParser();
        runner.setProperty(GetCosmosDocumentRecord.QUERY, "select top 5 * from c");
        runner.setVariable("schema.name", "sample");
        runner.enqueue(new byte[] {});
        runner.run();
    ...
    public void testSortAndProjection() {
        JsonParser parser = new JsonParser();
        runner.setProperty(GetCosmosDocumentRecord.QUERY, "select c.id, c.category, c.payload from c order by c.id DESC");
        runner.setVariable("schema.name", "sample");
        runner.enqueue(new byte[] {});
        runner.run();
