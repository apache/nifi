...
public class GetCassandraTest {

    private TestRunner testRunner;
    private MockGetCassandra processor;

    @Before
    public void setUp() throws Exception {
        processor = new MockGetCassandra();
    ...
    public void testProcessorConfigValid() {
        ...
        testRunner.setProperty(GetCassandra.CQL_SELECT_QUERY, "select * from test");
    ...
    public void testProcessorELConfigValid() {
        ...
        testRunner.setProperty(GetCassandra.CQL_SELECT_QUERY, "${query}");
    ...
    public void testProcessorNoInputFlowFileAndExceptions() {
        ...
        testRunner.assertAllFlowFilesTransferred(GetCassandra.REL_SUCCESS, 1);
        testRunner.clearTransferState();

        // Test exceptions
        processor.setExceptionToThrow(new NoHostAvailableException(new HashMap<InetSocketAddress, Throwable>()));
        testRunner.run(1, true, true);
        testRunner.assertAllFlowFilesTransferred(GetCassandra.REL_RETRY, 1);
        testRunner.clearTransferState();
        ...
        testRunner.assertAllFlowFilesTransferred(GetCassandra.REL_FAILURE, 0);
        testRunner.clearTransferState();
    ...
    public void testProcessorJsonOutput() {
        ...
        // Test JSON output
        testRunner.setProperty(GetCassandra.OUTPUT_FORMAT, GetCassandra.JSON_FORMAT);
        testRunner.run(1, true, true);
        testRunner.assertAllFlowFilesTransferred(GetCassandra.REL_SUCCESS, 1);
        List<MockFlowFile> files = testRunner.getFlowFilesForRelationship(GetCassandra.REL_SUCCESS);
        assertNotNull(files);
    ...
    public void testProcessorELConfigJsonOutput() {
        ...
        testRunner.setProperty(GetCassandra.QUERY_TIMEOUT, "${timeout}");
        testRunner.setProperty(GetCassandra.FETCH_SIZE, "${fetch}");
        testRunner.setIncomingConnection(false);
        testRunner.assertValid();
    ...
    public void testProcessorJsonOutputWithQueryTimeout() {
        setUpStandardProcessorConfig();
        testRunner.setProperty(GetCassandra.QUERY_TIMEOUT, "5 sec");
        testRunner.setIncomingConnection(false);
    ...
    public void testProcessorEmptyFlowFileAndExceptions() {
        ...
        testRunner.enqueue("".getBytes());
        testRunner.run(1, true, true);
        testRunner.assertAllFlowFilesTransferred(GetCassandra.REL_FAILURE, 1);
        testRunner.clearTransferState();
    ...
    public void testCreateSchema() throws Exception {
        ...
        Schema schema = GetCassandra.createSchema(rs);
    ...
    public void testConvertToAvroStream() throws Exception {
        ...
        long numberOfRows = GetCassandra.convertToAvroStream(rs, baos, 0, null);
    ...
    public void testConvertToJSONStream() throws Exception {
        ResultSet rs = CassandraQueryTestUtil.createMockResultSet();
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        long numberOfRows = GetCassandra.convertToJsonStream(rs, baos, StandardCharsets.UTF_8, 0, null);
        assertEquals(2, numberOfRows);
    }

    @Test
    public void testConvertToJCSVStream() throws Exception {
        ResultSet rs = CassandraQueryTestUtil.createMockResultSet();
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        long numberOfRows = GetCassandra.convertToCSVStream(rs, baos, StandardCharsets.UTF_8, 0, null);
        assertEquals(2, numberOfRows);
    }
     ...
     */
    private static class MockGetCassandra extends GetCassandra {
