...
public abstract class SchemaRegistryRecordSetWriter extends SchemaRegistryService {
    ...
    private final List<AllowableValue> schemaWriteStrategyList = List.of(NO_SCHEMA, SCHEMA_NAME_ATTRIBUTE, AVRO_SCHEMA_ATTRIBUTE, CONFLUENT_ENCODED_SCHEMA);

    private final List<AllowableValue> schemaAccessStrategyList = List.of(INHERIT_RECORD_SCHEMA, SCHEMA_NAME_PROPERTY, SCHEMA_TEXT_PROPERTY);

    @Override
    public void migrateProperties(final PropertyConfiguration propertyConfiguration) {
        propertyConfiguration.removeProperty("schema-protocol-version");
    }
