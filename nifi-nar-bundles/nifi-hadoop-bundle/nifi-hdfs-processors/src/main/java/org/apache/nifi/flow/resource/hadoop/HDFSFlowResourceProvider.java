...
import org.apache.nifi.flow.resource.ImmutableFlowResourceDescriptor;
...
public class HDFSFlowResourceProvider implements FlowResourceProvider {
    private static final Logger LOGGER = LoggerFactory.getLogger(HDFSFlowResourceProvider.class);
    ...
    private volatile FlowResourceProviderInitializationContext context;
    ...
    @Override
    public Collection<FlowResourceDescriptor> listResources() throws IOException {
        if (!initialized) {
        ...
        try {
            ...
            final List<FlowResourceDescriptor> result = Arrays.stream(fileStatuses)
                .filter(fileStatus -> fileStatus.isFile())
                .map(fileStatus -> new ImmutableFlowResourceDescriptor(fileStatus.getPath().getName(), fileStatus.getModificationTime()))
                .filter(context.getFilter().orElseGet(() -> flowResourceDescriptor -> true))
                .collect(Collectors.toList());

            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("The following files were found: " + String.join(", ", result.stream().map(FlowResourceDescriptor::getFileName).collect(Collectors.toList())));
            }
    ...
    @Override
    public InputStream fetchExternalResource(String location) throws IOException {
        if (!initialized) {
            LOGGER.error("Provider is not initialized");
        }
