...
public class MockKuduScan extends ScanKudu {
    ...
    @Override
    public AsyncKuduClient buildClient(final String masters, ProcessContext context) {
        final AsyncKuduClient client = mock(AsyncKuduClient.class);

        try {
            when(client.openTable(anyString())).thenReturn(Deferred.fromResult(mock(KuduTable.class)));
