...
public class BulletinEnumerator implements Enumerator<Object> {
    ...
    public BulletinEnumerator(final boolean isClustered, final String clusterNodeIdentifier, final BulletinRepository bulletinRepository,
    ...
    public void reset() {
        if(bulletinRepository != null) {
            List<Bulletin> fullBulletinList = new ArrayList<>(bulletinRepository.findBulletinsForController());
            fullBulletinList.addAll(bulletinRepository.findBulletins((new BulletinQuery.Builder()).sourceType(ComponentType.PROCESSOR).build()));
            fullBulletinList.addAll(bulletinRepository.findBulletins((new BulletinQuery.Builder()).sourceType(ComponentType.INPUT_PORT).build()));
            fullBulletinList.addAll(bulletinRepository.findBulletins((new BulletinQuery.Builder()).sourceType(ComponentType.OUTPUT_PORT).build()));
            fullBulletinList.addAll(bulletinRepository.findBulletins((new BulletinQuery.Builder()).sourceType(ComponentType.REMOTE_PROCESS_GROUP).build()));
            fullBulletinList.addAll(bulletinRepository.findBulletins((new BulletinQuery.Builder()).sourceType(ComponentType.REPORTING_TASK).build()));
            fullBulletinList.addAll(bulletinRepository.findBulletins((new BulletinQuery.Builder()).sourceType(ComponentType.CONTROLLER_SERVICE).build()));
