...
(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        define(['jquery',
            ...
            function ($, nfErrorHandler, nfCommon, nfDialog, nfStorage, nfClient, nfControllerService, nfControllerServices, nfUniversalCapture, nfCustomUi) {
                return (nf.FlowAnalysisRule = factory($, nfErrorHandler, nfCommon, nfDialog, nfStorage, nfClient, nfControllerService, nfControllerServices, nfUniversalCapture, nfCustomUi));
            });
    ...
    var marshalDetails = function () {
        ...
        flowAnalysisRuleDto['ruleType'] = ruleType;
     ...
     */
    var getFlowAnalysisRulePropertyDescriptor = function (propertyName) {
    ...
    var nfFlowAnalysisRule = {
            ...
            $.when(reloadTask, loadHistory).done(function (taskResponse, historyResponse) {
                ...
                $('#flow-analysis-rule-type-combo').combo({
                    options: [{
                        ...
                        description: 'Treat analysis results by this rule as recommendations.'
                    }, {
                        text: 'Policy',
                        value: 'POLICY',
                        description: 'Treat analysis results by this rule as policies.'
                    }],
                    selectedOption: {
                        value: flowAnalysisRule['ruleType']
                    }
                });
        ...
        showDetails: function (flowAnalysisRuleEntity) {
            ...
            $.when(reloadTask, loadHistory).done(function (taskResponse, historyResponse) {
                ...
                var ruleType = flowAnalysisRule['ruleType'];
                if (ruleType === 'POLICY') {
                    ruleType = 'Policy';
                } else {
                    ruleType = 'Recommendation';
                }
                nfCommon.populateField('read-only-flow-analysis-rule-type', ruleType);
