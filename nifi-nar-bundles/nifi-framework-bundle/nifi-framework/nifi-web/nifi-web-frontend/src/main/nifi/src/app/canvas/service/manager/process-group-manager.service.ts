...
export class ProcessGroupManager {
    ...
    private select() {
        return this.processGroupContainer.selectAll('g.process-groups').data(this.processGroups, function (d: any) {
            return d.id;
        });
    }
    ...
    private renderProcessGroups(entered: any) {
        ...
        const processGroup = entered.append('g')
        ...
        processGroup.filter(function (d: any) {
    ...
    private updateProcessGroups(updated: any): void {
        ...
        updated.each(function (this: any, processGroupData: any) {
            ...
            if (processGroup.classed('visible')) {
                if (details.empty()) {
                    details = processGroup.append('g')
                        .attr('class', 'process-group-details');
                    ...
                    const processGroupStatsLabel = details.append('g')
                    ...
                    const processGroupStatsValue = details.append('g')
                        .attr('transform', 'translate(95, 75)');

                    // queued value
                    const queuedText = processGroupStatsValue.append('text')
                        .attr('width', 180)
                        .attr('height', 10)
                        .attr('x', 4)
                        .attr('y', 5)
                    ...
                    // in value
                    const inText = processGroupStatsValue.append('text')
                        .attr('width', 180)
                        .attr('height', 10)
                        .attr('x', 4)
                        .attr('y', 24)
                    ...
                    // out value
                    const outText = processGroupStatsValue.append('text')
                        .attr('width', 180)
                        .attr('height', 10)
                        .attr('x', 4)
                        .attr('y', 60)
                    ...
                    // stats value container
                    const processGroupStatsInfo = details.append('g')
                        .attr('transform', 'translate(335, 75)');
                    ...
                    details.append('path')
                        ...
                        .attr('transform', 'translate(' + (processGroupData.dimensions.width - 2) + ', ' + (processGroupData.dimensions.height - 10) + ')')
                ...
                // update transmitting
                const transmitting = details.select('text.process-group-transmitting')
                ...
                const transmittingCount = details.select('text.process-group-transmitting-count')
                    .attr('x', function () {
                        const transmittingCountX = parseInt(transmitting.attr('x'), 10);
                        return transmittingCountX + Math.round(transmitting.node().getComputedTextLength()) + ProcessGroupManager.CONTENTS_VALUE_SPACER;
                    })
                    .text(function (d: any) {
                ...
                const notTransmitting = details.select('text.process-group-not-transmitting')
                    ...
                    .attr('x', function () {
                        ...
                        return transmittingX + Math.round(transmittingCount.node().getComputedTextLength()) + ProcessGroupManager.CONTENTS_SPACER;
                    });
                const notTransmittingCount = details.select('text.process-group-not-transmitting-count')
                    .attr('x', function () {
                        const notTransmittingCountX = parseInt(notTransmitting.attr('x'), 10);
                        return notTransmittingCountX + Math.round(notTransmitting.node().getComputedTextLength()) + ProcessGroupManager.CONTENTS_VALUE_SPACER;
                    })
                    .text(function (d: any) {
                ...
                // update running
                const running = details.select('text.process-group-running')
                    ...
                    .attr('x', function () {
                        ...
                        return notTransmittingX + Math.round(notTransmittingCount.node().getComputedTextLength()) + ProcessGroupManager.CONTENTS_SPACER;
                    });
                const runningCount = details.select('text.process-group-running-count')
                    .attr('x', function () {
                        const runningCountX = parseInt(running.attr('x'), 10);
                        return runningCountX + Math.round(running.node().getComputedTextLength()) + ProcessGroupManager.CONTENTS_VALUE_SPACER;
                    })
                    .text(function (d: any) {
                ...
                // update stopped
                const stopped = details.select('text.process-group-stopped')
                    ...
                    .attr('x', function () {
                        ...
                        return runningX + Math.round(runningCount.node().getComputedTextLength()) + ProcessGroupManager.CONTENTS_SPACER;
                    });
                const stoppedCount = details.select('text.process-group-stopped-count')
                    .attr('x', function () {
                        const stoppedCountX = parseInt(stopped.attr('x'), 10);
                        return stoppedCountX + Math.round(stopped.node().getComputedTextLength()) + ProcessGroupManager.CONTENTS_VALUE_SPACER;
                    })
                    .text(function (d: any) {
                ...
                // update invalid
                const invalid = details.select('text.process-group-invalid')
                    ...
                    .attr('x', function () {
                        ...
                        return stoppedX + Math.round(stoppedCount.node().getComputedTextLength()) + ProcessGroupManager.CONTENTS_SPACER;
                    });
                const invalidCount = details.select('text.process-group-invalid-count')
                    .attr('x', function () {
                        const invalidCountX = parseInt(invalid.attr('x'), 10);
                        return invalidCountX + Math.round(invalid.node().getComputedTextLength()) + ProcessGroupManager.CONTENTS_VALUE_SPACER;
                    })
                    .text(function (d: any) {
                ...
                const disabled = details.select('text.process-group-disabled')
                    ...
                    .attr('x', function () {
                        ...
                        return invalidX + Math.round(invalidCount.node().getComputedTextLength()) + ProcessGroupManager.CONTENTS_SPACER;
                    });
                const disabledCount = details.select('text.process-group-disabled-count')
                    .attr('x', function () {
                        const disabledCountX = parseInt(disabled.attr('x'), 10);
                        return disabledCountX + Math.round(disabled.node().getComputedTextLength()) + ProcessGroupManager.CONTENTS_VALUE_SPACER;
                    })
                    .text(function (d: any) {
                ...
                // up to date current
                const upToDate = details.select('text.process-group-up-to-date')
                ...
                const upToDateCount = details.select('text.process-group-up-to-date-count')
                    .attr('x', function () {
                        const updateToDateCountX = parseInt(upToDate.attr('x'), 10);
                        return updateToDateCountX + Math.round(upToDate.node().getComputedTextLength()) + ProcessGroupManager.CONTENTS_VALUE_SPACER;
                    })
                    .text(function (d: any) {
                ...
                const locallyModified = details.select('text.process-group-locally-modified')
                    ...
                    .attr('x', function () {
                        ...
                        return upToDateX + Math.round(upToDateCount.node().getComputedTextLength()) + ProcessGroupManager.CONTENTS_SPACER;
                    });
                const locallyModifiedCount = details.select('text.process-group-locally-modified-count')
                    .attr('x', function () {
                        const locallyModifiedCountX = parseInt(locallyModified.attr('x'), 10);
                        return locallyModifiedCountX + Math.round(locallyModified.node().getComputedTextLength()) + ProcessGroupManager.CONTENTS_VALUE_SPACER;
                    })
                    .text(function (d: any) {
                ...
                // update stale
                const stale = details.select('text.process-group-stale')
                    ...
                    .attr('x', function () {
                        ...
                        return locallyModifiedX + Math.round(locallyModifiedCount.node().getComputedTextLength()) + ProcessGroupManager.CONTENTS_SPACER;
                    });
                const staleCount = details.select('text.process-group-stale-count')
                    .attr('x', function () {
                        const staleCountX = parseInt(stale.attr('x'), 10);
                        return staleCountX + Math.round(stale.node().getComputedTextLength()) + ProcessGroupManager.CONTENTS_VALUE_SPACER;
                    })
                    .text(function (d: any) {
                        return d.staleCount;
                    });
                ...
                const locallyModifiedAndStale = details.select('text.process-group-locally-modified-and-stale')
                    ...
                    .attr('x', function () {
                        ...
                        return staleX + Math.round(staleCount.node().getComputedTextLength()) + ProcessGroupManager.CONTENTS_SPACER;
                    });
                const locallyModifiedAndStaleCount = details.select('text.process-group-locally-modified-and-stale-count')
                    .attr('x', function () {
                        const locallyModifiedAndStaleCountX = parseInt(locallyModifiedAndStale.attr('x'), 10);
                        return locallyModifiedAndStaleCountX + Math.round(locallyModifiedAndStale.node().getComputedTextLength()) + ProcessGroupManager.CONTENTS_VALUE_SPACER;
                    })
                    .text(function (d: any) {
                ...
                // update sync failure
                const syncFailure = details.select('text.process-group-sync-failure')
                    ...
                    .attr('x', function () {
                        ...
                        return syncFailureX + Math.round(locallyModifiedAndStaleCount.node().getComputedTextLength()) + ProcessGroupManager.CONTENTS_SPACER - 2;
                    });
                const syncFailureCount = details.select('text.process-group-sync-failure-count')
                    .attr('x', function () {
                        const syncFailureCountX = parseInt(syncFailure.attr('x'), 10);
                        return syncFailureCountX + Math.round(syncFailure.node().getComputedTextLength()) + ProcessGroupManager.CONTENTS_VALUE_SPACER;
                    })
                    .text(function (d: any) {
                ...
                const versionControl = processGroup.select('text.version-control')
                ...
                if (processGroupData.permissions.canRead) {
                    ...
                    processGroup.select('path.component-comments')
                        .style('visibility', self.canvasUtils.isBlank(processGroupData.component.comments) ? 'hidden' : 'visible')
                        .each(function () {
                            // get the tip
                    ...
                    processGroup.select('text.process-group-name')
                        .attr('x', function () {
                            if (self.isUnderVersionControl(processGroupData)) {
                                ...
                                return versionControlX + Math.round(versionControl.node().getComputedTextLength()) + ProcessGroupManager.CONTENTS_VALUE_SPACER;
                            } else {
                                return 10;
                            }
                        })
                    ...
                    processGroup.select('text.process-group-name')
                        .text(function (d: any) {
                            ...
                            if (name.length > ProcessGroupManager.PREVIEW_NAME_LENGTH) {
                                return name.substring(0, ProcessGroupManager.PREVIEW_NAME_LENGTH) + String.fromCharCode(8230);
                            } else {
                                return name;
                            }
                        });
    ...
    private updateProcessGroupStatus(updated: any) {
        ...
        updated.select('text.process-group-out tspan.count')
            .text(function (d: any) {
                return ' ' + String.fromCharCode(8594) + ' ' + self.canvasUtils.substringBeforeFirst(d.status.aggregateSnapshot.output, ' ');
            });
    ...
    public init(): void {
        this.processGroupContainer = d3.select('#canvas').append('g')
            .attr('pointer-events', 'all')
            .attr('class', 'process-groups');

        this.store.pipe(select(selectProcessGroups))
    ...
    private set(processGroups: any): void {
        ...
        // exit
        selection.exit().call(this.removeProcessGroups);
    }
