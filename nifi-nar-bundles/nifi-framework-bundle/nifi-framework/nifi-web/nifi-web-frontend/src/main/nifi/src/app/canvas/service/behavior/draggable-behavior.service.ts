...
export class DraggableBehavior {
  ...
  constructor(
    ...
    this.store.pipe(select(selectTransform))
      .subscribe(transform => {
    ...
    this.drag = d3.drag()
        ...
        .on('drag', function (event) {
          ...
          if (dragSelection.empty()) {
            ...
            dragSelection.attr('x', function (d: any) {
              d.x += event.dx;
              return self.snapEnabled ? (Math.round(d.x / self.snapAlignmentPixels) * self.snapAlignmentPixels) : d.x;
            }).attr('y', function (d: any) {
              d.y += event.dy;
              return self.snapEnabled ? (Math.round(d.y / self.snapAlignmentPixels) * self.snapAlignmentPixels) : d.y;
            });
          }
        })
