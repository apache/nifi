...
public class BufferingFlowResourceProviderWorkerTest {
    ...
    @Mock
    private FlowResourceConflictResolutionStrategy conflictResolutionStrategy;

    private Actions actions;
    private CountDownLatch countDownLatch;
    private TestFlowResourceProvider provider;
    private BufferingFlowResourceProviderWorker testSubject;

    @Before
    public void setUp() throws IOException {
        Mockito.when(conflictResolutionStrategy.shouldBeFetched(Mockito.any(File.class), Mockito.any(FlowResourceDescriptor.class))).thenReturn(true);
    ...
    public void testHappyPathWhenThereAreResources() throws InterruptedException, IOException {
        ...
        final FlowResourceDescriptor resource1 = givenAvailableResource(RESOURCE_NAME_1);
        final FlowResourceDescriptor resource2 = givenAvailableResource(RESOURCE_NAME_2);
    ...
    public void testExistingFileIsReplaced() throws InterruptedException, IOException {
        ...
        final FlowResourceDescriptor resource = givenAvailableResource(RESOURCE_NAME_1);
    ...
    private FlowResourceDescriptor givenAvailableResource(String resourceName) {
        final FlowResourceDescriptor resource = new ImmutableFlowResourceDescriptor(resourceName, System.currentTimeMillis());
        return resource;
    }

    private void givenTestSubject() {
        testSubject = new BufferingFlowResourceProviderWorker(PREFIX, this.getClass().getClassLoader(), provider, conflictResolutionStrategy, TARGET_DIRECTORY, 50, new CountDownLatch(0));
    }

    private void givenProviderWithResourceAndFetchSuccessSequence(final FlowResourceDescriptor resource, final Boolean... fetchSuccessSequence) {
        countDownLatch = new CountDownLatch(actions.numberOfAllActions());
        provider = new TestFlowResourceProvider(Collections.singletonList(resource), countDownLatch, Arrays.asList(fetchSuccessSequence));
    }

    private void givenProviderWithResource(final FlowResourceDescriptor... resources) {
        countDownLatch = new CountDownLatch(actions.numberOfAllActions());
        provider = new TestFlowResourceProvider(Arrays.asList(resources), countDownLatch);
    }
    ...
    private void givenResourceShouldNotBeFetched(final FlowResourceDescriptor resource) {
    ...
    private void thenConflictResulotionWasNotNeeded() {
        Mockito.verify(conflictResolutionStrategy, Mockito.never()).shouldBeFetched(Mockito.any(File.class), Mockito.any(FlowResourceDescriptor.class));
    }

    private void thenConflictResulotionWasNeeded(final FlowResourceDescriptor resource, final int times) {
    ...
    private void thenTargetFolderContains(final String content, final FlowResourceDescriptor... resources) throws IOException {
        Assert.assertEquals(resources.length, TARGET_DIRECTORY.list().length);

        for (final FlowResourceDescriptor resource : resources) {
            File acquiredResource = new File(TARGET_DIRECTORY_PATH, resource.getFileName());
            Assert.assertTrue(acquiredResource.exists());
            thenAssertFileContains(acquiredResource, content);
        }
    }

    private void thenTargetFolderContains(final String content, final FlowResourceDescriptor resource) throws IOException {
        thenTargetFolderContains(content, resource.getFileName());
    }
    ...
    private static class TestFlowResourceProvider implements FlowResourceProvider {
        private final List<FlowResourceDescriptor> resources;
        ...
        private TestFlowResourceProvider(final List<FlowResourceDescriptor> resources, final CountDownLatch countDownLatch) {
            this.countDownLatch = Optional.of(countDownLatch);
            this.resources = resources;
            this.fetchSuccessSequence = Collections.singletonList(true);
        }

        private TestFlowResourceProvider(final List<FlowResourceDescriptor> resources, final CountDownLatch countDownLatch, List<Boolean> fetchSuccessSequence) {
            this.countDownLatch = Optional.of(countDownLatch);
            this.resources = resources;
        ...
        @Override
        public void initialize(final FlowResourceProviderInitializationContext context) {

        }

        @Override
        public Collection<FlowResourceDescriptor> listResources() {
        ...
        @Override
        public InputStream fetchExternalResource(final String location) {
