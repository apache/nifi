...
/**
 * It is important to note that this implementation prevents NiFi from catching up a file under writing. Trying to load a NAR
 ...
 */
final class BufferingFlowResourceProviderWorker extends ConflictResolvingFlowResourceProviderWorker {
    private static final Logger LOGGER = LoggerFactory.getLogger(BufferingFlowResourceProviderWorker.class);

    BufferingFlowResourceProviderWorker(
    ...
    protected void acquireResource(final FlowResourceDescriptor availableResource) throws IOException {
        final long startedAt = System.currentTimeMillis();
        final InputStream inputStream;

        final File targetFile = new File(getTargetDirectory(), availableResource.getFileName());
        ...
        try (final NarCloseable narCloseable = NarCloseable.withComponentNarLoader(getProviderClassLoader())) {
            inputStream = getProvider().fetchExternalResource(availableResource.getFileName());
        }

        if (bufferFile.exists() && !bufferFile.delete()) {
            throw new FlowResourceProviderException("Buffer file '" + bufferFile.getName() +"' already exists and cannot be deleted");
        }
        ...
        if (targetFile.exists() && !targetFile.renameTo(setAsideFile)) {
            throw new FlowResourceProviderException("Target file '" + targetFile.getName() +"' already exists and cannot be moved aside");
        }

        if (bufferFile.renameTo(targetFile)) {
            LOGGER.info("Downloaded flow resource {} in {} ms", availableResource.getFileName(), (System.currentTimeMillis() - startedAt));

            if (setAsideFile.exists() && !setAsideFile.delete()) {
                LOGGER.error("Could not remove set aside file for {}", targetFile.getName());
            }
        } else {
            LOGGER.error("Could not put downloaded resource {} in place", availableResource.getFileName());
