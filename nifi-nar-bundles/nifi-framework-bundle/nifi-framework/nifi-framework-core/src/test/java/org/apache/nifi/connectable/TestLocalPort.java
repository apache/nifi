...
public class TestLocalPort {
    ...
    public void testSetThreads() {
        LocalPort port = getLocalPort("nifi.funnel.max.concurrent.tasks", "2");
    ...
    public void testSetTasks() {
        {
            LocalPort port = getLocalPort("nifi.funnel.max.transferred.flowfiles", "100000");
            assertEquals(1, port.getMaxConcurrentTasks());
            assertEquals(100, port.maxIterations);
        }
        {
            LocalPort port = getLocalPort("nifi.funnel.max.transferred.flowfiles", "100001");
            assertEquals(1, port.getMaxConcurrentTasks());
            assertEquals(101, port.maxIterations);
        }
        {
            LocalPort port = getLocalPort("nifi.funnel.max.transferred.flowfiles", "99999");
            assertEquals(1, port.getMaxConcurrentTasks());
            assertEquals(100, port.maxIterations);
        }
        {
            LocalPort port = getLocalPort("nifi.funnel.max.transferred.flowfiles", "0");
            assertEquals(1, port.getMaxConcurrentTasks());
            assertEquals(1, port.maxIterations);
        }
        {
            LocalPort port = getLocalPort("nifi.funnel.max.transferred.flowfiles", "1");
            assertEquals(1, port.getMaxConcurrentTasks());
            assertEquals(1, port.maxIterations);
        }
    }
