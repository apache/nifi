...
final class StorateStatusInsertRowDataSource implements InsertRowDataSource {
    private final Iterator<StorageStatusStatistic> statuses;

    private StorateStatusInsertRowDataSource(final Iterator<StorageStatusStatistic> statuses) {
    ...
    static InsertRowDataSource getInstance(final Collection<NodeStatus> nodeStatuses) {
        final List<StorageStatusStatistic> statuses = new ArrayList<>();
        for (final NodeStatus nodeStatus : nodeStatuses) {
            final Instant capturedAt = Instant.ofEpochMilli(nodeStatus.getCreatedAtInMs());
            nodeStatus.getContentRepositories().forEach(storageStatus -> statuses.add(new StorageStatusStatistic(storageStatus, StorageStatusType.CONTENT, capturedAt)));
            nodeStatus.getProvenanceRepositories().forEach(storageStatus -> statuses.add(new StorageStatusStatistic(storageStatus, StorageStatusType.PROVENANCE, capturedAt)));
        }

        return new StorateStatusInsertRowDataSource(statuses.iterator());
    }
