...
@CapabilityDescription("Consumes messages from Apache Kafka specifically built against the Kafka 3.0 Consumer API. "
    + "The complementary NiFi processor for sending messages is PublishKafka_3_0.")
@Tags({"Kafka", "Get", "Ingest", "Ingress", "Topic", "PubSub", "Consume", "3.0"})
@WritesAttributes({
...
public class ConsumeKafka_3_0 extends AbstractProcessor implements VerifiableProcessor {
    ...
    protected ConsumerPool createConsumerPool(final ProcessContext context, final ComponentLog log) {
        ...
        final byte[] demarcator = context.getProperty(ConsumeKafka_3_0.MESSAGE_DEMARCATOR).isSet()
                ? context.getProperty(ConsumeKafka_3_0.MESSAGE_DEMARCATOR).evaluateAttributeExpressions().getValue().getBytes(StandardCharsets.UTF_8) : null;
        ...
        final String topicListing = context.getProperty(ConsumeKafka_3_0.TOPICS).evaluateAttributeExpressions().getValue();
        final String topicType = context.getProperty(ConsumeKafka_3_0.TOPIC_TYPE).evaluateAttributeExpressions().getValue();
