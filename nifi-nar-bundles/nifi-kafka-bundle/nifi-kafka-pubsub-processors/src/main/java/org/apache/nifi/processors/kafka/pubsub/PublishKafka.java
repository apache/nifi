...
public class PublishKafka extends AbstractKafkaProcessor<KafkaPublisher> {
    ...
    static final PropertyDescriptor TOPIC = TOPIC_BUILDER.expressionLanguageSupported(true).build();
    ...
    protected boolean rendezvousWithKafka(ProcessContext context, ProcessSession session) throws ProcessException {
        ...
        if (flowFile != null) {
            flowFile = this.process(flowFile, context, session);
            if (!this.isFailedFlowFile(flowFile)) {
                session.getProvenanceReporter().send(flowFile, context.getProperty(BOOTSTRAP_SERVERS).getValue() + "/" + TOPIC.getName());
     ...
     *
     */
    private FlowFile process(final FlowFile flowFile, final ProcessContext context, final ProcessSession session) {
        ...
        session.read(flowFile, new InputStreamCallback() {
            ...
            public void process(InputStream contentStream) throws IOException {
                ...
                if (PublishKafka.this.isFailedFlowFile(flowFile)) {
                    ...
                    delimiterBytes = context.getProperty(MESSAGE_DEMARCATOR).isSet()
                            ? context.getProperty(MESSAGE_DEMARCATOR).evaluateAttributeExpressions(flowFile).getValue().getBytes(StandardCharsets.UTF_8) : null;
                }
        ...
        KafkaPublisherResult publishResult = publishResultRef.get();

        FlowFile resultFile = publishResult.isAllAcked()
                ? PublishKafka.this.cleanUpFlowFileIfNecessary(flowFile, session)
                : session.putAllAttributes(flowFile, this.buildFailedFlowFileAttributes(publishResult.getLastMessageAcked(), flowFile, context));
    ...
    private FlowFile cleanUpFlowFileIfNecessary(FlowFile flowFile, ProcessSession session) {
        if (this.isFailedFlowFile(flowFile)) {
            flowFile = session.removeAttribute(flowFile, FAILED_DELIMITER_ATTR);
            flowFile = session.removeAttribute(flowFile, FAILED_KEY_ATTR);
            flowFile = session.removeAttribute(flowFile, FAILED_TOPIC_ATTR);
            flowFile = session.removeAttribute(flowFile, FAILED_PROC_ID_ATTR);
            flowFile = session.removeAttribute(flowFile, FAILED_LAST_ACK_IDX);
        }
        return flowFile;
    }
    ...
    */
    ...
    private boolean isFailedFlowFile(FlowFile flowFile) {
        return PublishKafka.this.getIdentifier().equals(flowFile.getAttribute(FAILED_PROC_ID_ATTR));
    }
}
