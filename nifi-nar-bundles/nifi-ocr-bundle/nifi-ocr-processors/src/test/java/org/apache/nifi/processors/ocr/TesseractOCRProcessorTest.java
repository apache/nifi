...
public class TesseractOCRProcessorTest {
    ...
    public void testNonExistentTesseractInstallation() throws Exception {
        testRunner.setProperty(TesseractOCRProcessor.TESSERACT_INSTALL_PATH, "/invalid/path/to/no/tesseract");
        testRunner.setProperty(TesseractOCRProcessor.TESSERACT_LANGUAGE, "zzz");

        testRunner.enqueue(new File("src/test/resources/QuickBrownFox.jpg").toPath());
        testRunner.run();
        ...
        testRunner.assertTransferCount(TesseractOCRProcessor.REL_SUCCESS, 5);
    }
