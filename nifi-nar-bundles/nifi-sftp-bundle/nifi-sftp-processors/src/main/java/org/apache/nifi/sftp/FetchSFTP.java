...
@SeeAlso({GetSFTP.class, PutSFTP.class})
@WritesAttributes({
...
public class FetchSFTP extends FetchFileTransfer {
    ...
    protected List<PropertyDescriptor> getSupportedPropertyDescriptors() {
        final PropertyDescriptor port = new PropertyDescriptor.Builder().fromPropertyDescriptor(FetchFileTransfer.UNDEFAULTED_PORT).defaultValue("22").build();
        final PropertyDescriptor disableDirectoryListing = new PropertyDescriptor.Builder()
                ...
                .description(String.format("Control how '%s' is created when '%s' is '%s' and '%s' is enabled. %s",
                        FetchFileTransfer.MOVE_DESTINATION_DIR.getDisplayName(),
                        FetchFileTransfer.COMPLETION_STRATEGY.getDisplayName(),
                        FetchFileTransfer.COMPLETION_MOVE.getDisplayName(),
                        FetchFileTransfer.MOVE_CREATE_DIRECTORY.getDisplayName(),
                        SFTPTransfer.DISABLE_DIRECTORY_LISTING.getDescription())).build();

        final List<PropertyDescriptor> properties = new ArrayList<>();
        properties.add(FetchFileTransfer.HOSTNAME);
        properties.add(port);
        properties.add(FetchFileTransfer.USERNAME);
        ...
        properties.add(FetchFileTransfer.COMPLETION_STRATEGY);
        properties.add(FetchFileTransfer.MOVE_DESTINATION_DIR);
        properties.add(FetchFileTransfer.MOVE_CREATE_DIRECTORY);
        ...
        properties.add(FetchFileTransfer.FILE_NOT_FOUND_LOG_LEVEL);
