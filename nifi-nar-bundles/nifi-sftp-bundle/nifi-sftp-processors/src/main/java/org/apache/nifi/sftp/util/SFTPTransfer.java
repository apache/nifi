 ...
 */
package org.apache.nifi.sftp.util;
...
import org.apache.nifi.sftp.ssh.SSHClientProvider;
import org.apache.nifi.sftp.ssh.StandardSSHClientProvider;
...
public class SFTPTransfer implements FileTransfer {
    ...
    protected SFTPClient getSFTPClient(final FlowFile flowFile) throws IOException {
        final String evaledHostname = ctx.getProperty(FileTransfer.HOSTNAME).evaluateAttributeExpressions(flowFile).getValue();
        final String evaledPort = ctx.getProperty(PORT).evaluateAttributeExpressions(flowFile).getValue();
        final String evaledUsername = ctx.getProperty(FileTransfer.USERNAME).evaluateAttributeExpressions(flowFile).getValue();
        final String evaledPassword = ctx.getProperty(FileTransfer.PASSWORD).evaluateAttributeExpressions(flowFile).getValue();
    ...
    public String put(final FlowFile flowFile, final String path, final String filename, final InputStream content) throws IOException {
        ...
        String tempFilename = ctx.getProperty(FileTransfer.TEMP_FILENAME).evaluateAttributeExpressions(flowFile).getValue();
        if (tempFilename == null) {
            final boolean dotRename = ctx.getProperty(FileTransfer.DOT_RENAME).asBoolean();
        ...
        int perms;
        final String permissions = ctx.getProperty(FileTransfer.PERMISSIONS).evaluateAttributeExpressions(flowFile).getValue();
        ...
        final String lastModifiedTime = ctx.getProperty(FileTransfer.LAST_MODIFIED_TIME).evaluateAttributeExpressions(flowFile).getValue();
        if (lastModifiedTime != null && !lastModifiedTime.trim().isEmpty()) {
            try {
                final DateTimeFormatter dateTimeFormatter = DateTimeFormatter.ofPattern(FileTransfer.FILE_MODIFY_DATE_ATTR_FORMAT, Locale.US);
        ...
        final String owner = ctx.getProperty(FileTransfer.REMOTE_OWNER).evaluateAttributeExpressions(flowFile).getValue();
        ...
        final String group = ctx.getProperty(FileTransfer.REMOTE_GROUP).evaluateAttributeExpressions(flowFile).getValue();
