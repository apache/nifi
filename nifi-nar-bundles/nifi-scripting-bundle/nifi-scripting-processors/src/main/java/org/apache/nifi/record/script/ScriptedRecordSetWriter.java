...
public class ScriptedRecordSetWriter extends AbstractScriptedRecordFactory<RecordSetWriterFactory> implements RecordSetWriterFactory {
    ...
    @Override
    public RecordSetWriter createWriter(Map<String, String> variables, ComponentLog logger, RecordSchema schema, OutputStream out) throws SchemaNotFoundException, IOException {
        if (recordFactory.get() != null) {
            try {
                return recordFactory.get().createWriter(variables, logger, schema, out);
