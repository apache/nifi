...
public class PutBigQueryBatch extends AbstractBigQueryProcessor {
    ...
    private static final List<String> TYPES = Arrays.asList(FormatOptions.json().getType(),
            FormatOptions.csv().getType(), FormatOptions.avro().getType(),FormatOptions.parquet().getType(),
            FormatOptions.orc().getType(),FormatOptions.datastoreBackup().getType());
