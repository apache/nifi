...
public class FnProcessorInitializationContext implements ProcessorInitializationContext {
    ...
    public FnProcessorInitializationContext(final String id, final Processor processor, final ProcessContext context) {
        processorId = id;
        logger = new SLF4JComponentLog(processor);
        this.controllerServiceLookup = context.getControllerServiceLookup();
    }

    public FnProcessorInitializationContext(final String id, final Processor processor, final ControllerServiceLookup controllerServiceLookup) {
        processorId = id;
        logger = new SLF4JComponentLog(processor);
