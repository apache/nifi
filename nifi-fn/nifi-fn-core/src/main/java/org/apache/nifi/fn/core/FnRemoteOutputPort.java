...
public class FnRemoteOutputPort extends AbstractFnComponent {
    ...
    public FnRemoteOutputPort(final VersionedRemoteProcessGroup rpg, final VersionedRemoteGroupPort remotePort, final SSLContext sslContext) {
    ...
    public boolean runRecursive(final Queue<InMemoryFlowFile> queue) {
        try {
            ...
            final Queue<FnFlowFile> destinationQueue = new LinkedList<>();
            ...
            while ((dataPacket = transaction.receive()) != null) {
                ...
                final FnFlowFile receivedFlowFile = new FnFlowFile(buffer, attributes, true);
                destinationQueue.add(receivedFlowFile);

                for (final FnComponent childComponent : getChildren().get(Relationship.ANONYMOUS)) {
