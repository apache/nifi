...
import org.apache.nifi.fn.bootstrap.ExtensionDiscovery;
...
public class FnFlow implements RunnableFlow {
    ...
    private List<FnComponent> roots;
    private volatile boolean stopRequested = false;
    private FnComponent sourceComponent = null;

    private final ComponentFactory componentFactory;

    public FnFlow(final FnProcessorWrapper root) {
        this(Collections.singletonList(root));
    }

    public FnFlow(final List<FnComponent> roots) {
        this.roots = roots;
        this.componentFactory = null;
    }


    public FnFlow(final VersionedProcessGroup flow, final ExtensionManager extensionManager, final VariableRegistry variableRegistry,
        ...
        final Map<String, FnComponent> componentMap = new HashMap<>();
        ...
        for (final VersionedConnection connection : connections) {
            ...
            FnComponent sourceComponent = null;
            if (componentMap.containsKey(source.getId())) {
                ...
                switch (source.getType()) {
                    ...
                    case REMOTE_OUTPUT_PORT:
                        ...
                        sourceComponent = new FnRemoteOutputPort(rpg, remotePort, sslContext);
                        componentMap.put(source.getId(), sourceComponent);
                        break;
                    case OUTPUT_PORT:
                    case FUNNEL:
                        sourceComponent = new FnPassThroughComponent();
                        componentMap.put(source.getId(), sourceComponent);
                        break;
                    case INPUT_PORT:
            ...
            FnComponent destinationComponent = null;
            switch (destination.getType()) {
                ...
                case REMOTE_INPUT_PORT:
                    if (componentMap.containsKey(destination.getId())) {
                        ...
                        destinationComponent = new FnRemoteInputPort(rpg, remotePort, sslContext);
                        destinationComponent.addParent(sourceComponent);
                        componentMap.put(destination.getId(), destinationComponent);
                    }
                ...
                case FUNNEL:
                    if (componentMap.containsKey(destination.getId())) {
                        ...
                        destinationComponent = new FnPassThroughComponent();
                        componentMap.put(destination.getId(), destinationComponent);
                    }
    ...
    public boolean run(final Queue<InMemoryFlowFile> output) {
        while (!this.stopRequested) {
            for (final FnComponent pw : roots) {
    ...
    public void shutdown() {
        ...
        this.roots.forEach(FnComponent::shutdown);
    }
    ...
    public static FnFlow createAndEnqueueFromJSON(final JsonObject args) throws InitializationException, IOException, ProcessorInstantiationException, NiFiRegistryException {
        return createAndEnqueueFromJSON(args, ClassLoader.getSystemClassLoader(), new File(DEFAULT_WORKING_DIR));
    }

    public static FnFlow createAndEnqueueFromJSON(final JsonObject args, final ClassLoader systemClassLoader, final File narWorkingDir)
        ...
        final FnFlow flow = new FnFlow(snapshot.getFlowContents(), extensionManager, () -> inputVariables, failurePorts, materializeContent, sslContext);
    ...
    public void enqueueFlowFile(final byte[] content, final Map<String, String> attributes) {
        ...
        //enqueue data
        final Queue<FnFlowFile> input = new LinkedList<>();
        input.add(new FnFlowFile(content, attributes, sourceComponent.isMaterializeContent()));
